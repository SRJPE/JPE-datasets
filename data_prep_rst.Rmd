...
filter(battle_rst, is.na(fork_length), count > 0)
filter(battle_rst, is.na(run))
# notes to include
...
battle_rst_clean <- battle_rst %>%
  ...
  mutate(tributary = "Battle Creek",
         site = "Battle Creek",
         run = case_when(is.na(run) ~ "not recorded",
...
butte_hours_fished <- butte_rst %>%
  ...
  select(start_datetime, end_datetime, station) %>%
...
filter(butte_rst, is.na(fork_length), count > 0)
...
filter(deer_rst, is.na(fork_length), count > 0)
...
filter(knights_landing_rst, run == "Not applicable (n/a)")
filter(knights_landing_rst, is.na(forkLength), n > 0)
...
knights_rst_clean <- knights_landing_rst %>%
  ...
  mutate(tributary = "Lower Sacramento - Knights Landing",
         site = "Lower Sacramento - Knights Landing",
         run = case_when(is.na(run) | run == "Not recorded" |  run == "Not applicable (n/a)" ~ "not recorded",
                         count == 0  ~ NA_character_,
                         T ~ tolower(run)),
         date = as_date(date))
```
...
filter(tisdale_rst, mark_type == "Pigment")
filter(tisdale_rst, is.na(run))
filter(tisdale_rst, is.na(fork_length_mm), count > 0)
filter(tisdale_rst, run == "unknown")
...
filter(mill_rst, is.na(fork_length), count > 0)
...
filter(yuba_rst, is.na(fork_length), count > 0)
filter(yuba_rst, is.na(run), count > 0)
filter(yuba_rst, run == "unknown")
