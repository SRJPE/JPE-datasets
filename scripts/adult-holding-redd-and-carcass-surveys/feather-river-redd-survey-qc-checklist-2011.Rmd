...
*No numeric values recorded for percent_boulder in 2011.
...
```
**Numeric summary of salmon_counted by location in 2011**
```{r}
cleaner_data_2011 %>%
  group_by(location) %>% 
  summarise(count = sum(salmon_counted, na.rm = T)) %>% 
...
No values recorded for percent_boulder
...
```
**Numeric Summary of velocity_m/s Over 2011**
```{r}
...
**NA and Unknown Values**
NA and Unknown Values**
* `r round(sum(is.na(cleaner_data_2011$'velocity_m/s'))/nrow(cleaner_data_2011), 3)*100` % of values in the `velocity_m/s` column are NA.
