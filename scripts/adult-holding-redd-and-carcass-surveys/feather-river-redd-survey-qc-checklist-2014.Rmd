...
Questionable redds - polygon mapped with Trimble GPS unit where the substrate was disturbed but did not have the proper characteristics to be called a redd - it was no longer recorded after 2014
...
```
**Numeric Daily Summary of salmon_counted Over 2014**
```{r}
cleaner_data_2014 %>%
  group_by(date) %>%
...
```
**Numeric summary of salmon_counted by location in 2014**
```{r}
cleaner_data_2014 %>%
  group_by(location) %>% 
  summarise(count = sum(salmon_counted, na.rm = T)) %>% 
...
* `r round(sum(is.na(cleaner_data_2014$salmon_counted))/nrow(cleaner_data_2014), 5)*100` % of values in the `salmon_counted` column are NA. 
...
```
**Numeric Summary of velocity_m/s Over 2014**
```{r}
...
**NA and Unknown Values**
NA and Unknown Values**
* `r round(sum(is.na(cleaner_data_2014$'velocity_m/s'))/nrow(cleaner_data_2014), 3)*100` % of values in the `velocity_m/s` column are NA.
