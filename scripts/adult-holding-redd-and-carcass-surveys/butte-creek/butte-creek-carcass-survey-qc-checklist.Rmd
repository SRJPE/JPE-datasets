...
read_from_cloud <- function(year){
  gcs_get_object(object_name = paste0("adult-holding-redd-and-carcass-surveys/butte-creek/data-raw/", year, "_SurveyChops.xls"),
               bucket = gcs_get_global_bucket(),
               saveToDisk = paste0(year,"_raw_surveychops.xls"),
               overwrite = TRUE)
  data <- readxl::read_excel(paste0(year,"_raw_surveychops.xls")) %>% 
    glimpse()
}
...
cleaner_data <- cleaner_data %>% 
  mutate(way_pt = tolower(way_pt),
         way_pt = if_else(way_pt == 'n/a', 'NA', way_pt),
         way_pt = if_else(way_pt == 'n/r', 'NA', way_pt))
table(cleaner_data$way_pt)
```
...
cleaner_data %>% 
  ...
  ggplot(aes(x = fake_date, y = chop_count))+
  theme_minimal()+
