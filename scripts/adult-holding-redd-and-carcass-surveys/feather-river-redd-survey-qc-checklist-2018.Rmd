...
Questionable redds - polygon mapped with Trimble GPS unit where the substrate was disturbed but did not have the proper characteristics to be called a redd - it was no longer recorded after 2018
...
**Numeric Daily Summary of salmon_counted Over 2018**\
```{r}
cleaner_data_2018 %>%
  group_by(date) %>%
...
**Numeric summary of salmon_counted by location in 2018**
```{r}
cleaner_data_2018 %>%
  group_by(location) %>% 
  summarise(count = sum(salmon_counted, na.rm = T)) %>% 
...
* `r round(sum(is.na(cleaner_data_2018$salmon_counted))/nrow(cleaner_data_2018), 5)*100` % of values in the `salmon_counted` column are NA. 
...
**NA and Unknown Values**
NA and Unknown Values**
* `r round(sum(is.na(cleaner_data_2018$'percent_boulder'))/nrow(cleaner_data_2018), 3)*100` % of values in the `percent_large_substrate` column are NA.
...
**Numeric Summary of velocity_m/s Over 2018**
```{r}
...
**NA and Unknown Values**
NA and Unknown Values**
* `r round(sum(is.na(cleaner_data_2018$'velocity_m/s'))/nrow(cleaner_data_2018), 3)*100` % of values in the `velocity_m/s` column are NA.
