...
rst_survey_variables <- bind_rows(battle_rst, butte_rst, clear_rst, deer_rst, feather_rst, mill_rst, yuba_rst, knights_rst, tisdale_rst) %>%
  group_by(variables) %>% 
  tally() %>% 
  glimpse()
...
```{r}
battle_rst <- readRDS("../../data/battle_rst_environmental_data_dictionary.rds") %>%
  mutate(location = "Battle Creek") %>% glimpse

# Butte all in one table 

clear_rst <- readRDS("../../data/clear_rst_environmental_data_dictionary.rds") %>% 
  mutate(location = "Clear Creek") %>% glimpse

# Deer all in one table 

feather_rst <- readRDS("../../data/feather_rst_effort_data_dictionary.rds") %>% 
  mutate(location = "Feather River") %>% glimpse()
...
knights_rst <- readRDS("../../data/lower_sac_knights_rst_effort_data_dictionary.rds") %>% 
  mutate(location = "Sac River - Knights") %>% glimpse
