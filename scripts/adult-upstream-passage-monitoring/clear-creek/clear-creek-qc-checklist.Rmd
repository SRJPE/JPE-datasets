...
* `r round(sum(is.na(cleaner_video_data$up))/nrow(cleaner_video_data), 3) * 100` % of values in the `up` column are NA. However, there are clearly gaps in data. More investigation needs to be done to see if 0 is a real 0 or if it can be explained by other factors (outages).
...
* `r round(sum(is.na(cleaner_video_data$down))/nrow(cleaner_video_data), 3) * 100` % of values in the `up` column are NA. 
...
* `r round(sum(cleaner_video_data$jack_size == "unknown", na.rm = T)/nrow(cleaner_video_data), 3) * 100` % of values in the `run` column are`unknown`.
