...
gcs_get_object(object_name = "adult-upstream-passage-monitoring/clear-creek/data-raw/ClearCreekVideoWeir_AdultRecruitment_2013-2020.xlsx",
...
`r round(sum(cleaner_video_data$adipose == "unknown")/nrow(cleaner_video_data), 3)` % of values in the `adipose` column are`unknown`.
...
`r round(sum(cleaner_video_data$sex == "unknown")/nrow(cleaner_video_data), 3)` % of values in the `sex` column are`unknown`.
...
`r round(sum(cleaner_video_data$spawning_condition == "unknown")/nrow(cleaner_video_data), 3)` % of values in the `spawning_condition` column are`unknown`.
...
`r round(sum(cleaner_video_data$jack_size == "unknown")/nrow(cleaner_video_data), 3)` % of values in the `jack_size` column are`unknown`.
