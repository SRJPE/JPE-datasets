...
# Gathering encodings for variables that have different encodings
redd_encodings <- full_join(redd_full, redd_survey_variables) %>%
  filter(n > 1, !is.na(encoding)) %>%
  select(variables, encoding) %>%
  pivot_wider(names_from = "variables", values_from = "encoding") %>%
  pivot_longer(everything(), names_to = "variables", values_to = "encoding_options")
...
redd_full_data_dictionary <- left_join(redd_survey_variables_names %>%
  ...
  left_join(redd_encodings) %>%
  mutate(encoding = case_when(encoding_options != "NULL" ~ as.character(encoding_options),
                              T ~ encoding)) %>%
  select(-encoding_options)
