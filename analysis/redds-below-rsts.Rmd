...
# swap out the problematic feather river points with representative locations for these same named reaches
default_redd_locations <- standard_daily_redd |> 
  filter(stream=="feather river" & year(date)>=2014 & st_coordinates(standard_daily_redd$geometry)[,1] < -121.6) |> 
  group_by(reach) |> 
  summarize() |> 
  st_centroid() |>
  rename(geometry_default = geometry) |> 
  as_tibble()
standard_daily_redd <- standard_daily_redd |> 
  left_join(default_redd_locations, by = join_by(reach)) |>
  mutate(geometry = if_else(stream=="feather river" & #!st_is_empty(geometry_default) &
                              (year(date) %in% c(NA, 2012, 2013) | (year(date)==2014 & st_coordinates(standard_daily_redd$geometry)[,1] > -121.6)), 
                            geometry_default, 
                            geometry)) |>
  select(-geometry_default) |>
  # remove any records that had invalid point geometries and that didn't have matches to the default redd locations
  filter(!st_is_empty(geometry))

# print final counts
cat("original count of redds including those with invalid geometries", nrow(redds_raw), "\n")
cat("final count after geometry corrections and filters", nrow(standard_daily_redd), "\n")
```
...
```{r message=FALSE, warning=FALSE}
  ...
  addCircleMarkers(data=standard_daily_redd, 
                   clusterOptions = markerClusterOptions(), 
                   label = ~paste(uid, stream, reach, year)) |>
...
if (!file.exists(file_path)) {
  ...
  st_geometry(redds_by_rst_catchment) <- redds_by_rst_catchment$geometry
...
## QC
#  leaflet() |> addTiles() |> 
#  addPolygons(data=catchments |> filter(rst_site=="shawns beach"), weight=1, fillOpacity=0, popup = ~rst_site) |>
#  addCircleMarkers(data=redds_by_rst_catchment |> filter(rst_site=="shawns beach" & stream=="feather river" & above_rst==FALSE), 
#                   clusterOptions = markerClusterOptions(), 
#                   label = ~paste(uid, stream, reach, year)) |>
#  addMarkers(data=rst_trap_locations, popup = ~rst_site) 
# QC
redds_by_rst_catchment |> filter(stream=="feather river") |> group_by(rst_site) |> summarize(mean(above_rst))
```
...
Verify these rsults... looking at the map there shouldn't be many at all below Shawn's Beach
...
testing fix swapping out coordinates for problematic points using representative locations of named redds

```{r}
redds_raw |> filter(stream=="feather river" & year(date) %in% c(2012, 2013)) |> ggplot() + geom_point(aes(x=longitude, y=latitude, color=as.factor(year(date))))
standard_daily_redd |> filter(stream=="feather river" & year(date) %in% c(2012, 2013)) |> ggplot() + geom_sf(aes(color=as.factor(year(date))))
```

This fix was applied to the original data sets where the reach name was able to be matched. Non-matching observations were dropped.
