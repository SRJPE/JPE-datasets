...
We explored three primary methods were used to calculate hours fished. Methods were chosen based on data collected at the RST site. When no times, trap revolutions, or trap RPMs were collected we assume 24 hours for each day that trap is fishing. The trap revolution method is commonly used by RST programs but we were not able to get reasonable results for most programs we tried it on so we ended up relying on trap times instead of revolutions. 
...
3) If only one date is available, assume that the end date is the date sampled following day.
...
feather_hours_fished <- filter(trap, stream == "feather river", 
                               ...
                               fish_processed == "processed fish", include == T, 
...
hours_fished_combined %>% filter(site == "lower feather river", trap_stop_date == as_date("2022-02-22")) 
```
