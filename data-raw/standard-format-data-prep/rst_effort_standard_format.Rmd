...
battle_hours_fished <- filter(trap, stream == "battle creek") %>%
  ...
  # fix typos 
  mutate(trap_start_date = case_when(trap_stop_date == "2004-02-13" ~ as_date("2004-02-12"),
...
clear_hours_fished <- filter(trap, stream == "clear creek") %>%
  ...
  # fix typos
  mutate(trap_start_time = case_when(trap_stop_date == "2020-02-24" ~ trap_stop_time,
                                     T ~ trap_start_time),
  ...
  filter(remove == "keep") %>% 
  select(-remove) %>% 
  mutate(trap_stop_time = case_when(site == "LCC" & trap_stop_date == "2021-05-10" ~ as_hms("08:00:00"),
                               T ~ trap_stop_time)) %>% 
  arrange(site, subsite, trap_stop_date, trap_stop_time) %>% 
  # fill in missing start dates with the stop date from previous day
...
tisdale_hours_fished <- filter(trap, stream == "sacramento river", site == "tisdale", fish_processed == "processed fish", include == T, visit_type != "start trapping") %>%
  ...
  arrange(subsite, trap_stop_date, trap_stop_time) %>% 
  # fix typos
  mutate(trap_stop_date = case_when(trap_start_date == "2019-07-24" ~ as.Date("2019-07-25"),
