...
-   `picket_weir_location`: numeric value ranging from 7.4 to 8.2; deemed unnecessary for final dataset because `picket_weir_relation` is included
...
clean_clear_redd <- clear_redd  |>
  select(surveyed_reach, date, river_mile, 
         ...
         pre_redd_substrate_size, redd_substrate_size, 
         tail_substrate_size, 
         velocity, picket_weir_relation, age,
         method)  |> # feet per second
  mutate(redd_count = 1, # each row is a redd observation
         stream = 'clear creek', 
         year = lubridate::year(date))  |>  
