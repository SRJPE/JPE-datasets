...
This dataset includes `annual_redd_count` which is the total number of redds sampled. This can be summarized by `redd_id` or by `max_yearly_redd_count` (maximum number of redds counted on a single day per year per reach).

Data dictionary for standard format for `annual_redd_count`:

|column_name                 |tributary_collects |definition                                                                                                |
|:---------------------------|:------------------|:---------------------------------------------------------------------------------------------------------|
|year                        |B, C, F, M, Y      |Calendar year sampling occurred                                                                           |
|stream                      |B, C, F, M, Y      |Unique stream associated with redd data collected                                                         |
|reach                       |B, C, F, M         |Unique name identifier of reach sampled                                                                   |
|species                     |B, C, Y      |Species associated with redd sampled                                                                      |
|run                         |B, C     |spring, fall, late_fall                                                                                   |
|annual_redd_count           |B, C, F, M, Y      |Number of redds counted in a year by reach and stream                                                     |
|annual_summarization_method |B, C, F, M, Y      |Summarization method used to get the annual number: either summed by redd id or maximum yearly redd count |
...
clean_battle_redd <- battle_redd  |>
  select(date, latitude, longitude, 
         reach, river_mile, fish_guarding, JPE_redd_id,
...
clean_feather_redd <- feather_redd  |>
  select(date, location, redd_count, 
         latitude, longitude, 
         redd_width_m, redd_length_m, velocity_m_per_s = velocity, depth_m)  |> 
  rename(reach = location,
...
all_redd_data <- bind_rows(clean_battle_redd, 
  ...
  mutate(species = case_when(species %in% c("Chinook", "chinook","CHN") ~ "chinook",
                             species %in% c("UNK", "Unknown", "Salmonid") ~ "unknown",
                             species %in% c("SH", "O. mykiss") ~ "steelhead",
                             species %in% c("Lamprey", "P. lamprey") ~ "pacific lamprey",
...
For Battle and Clear Creeks, the dataset was filtered to unique `redd_id` and these were summed by reach and year.
...
redd_count_by_year <- bind_rows(clean_feather_redd, 
                                clean_yuba_redd)  |> 
  # find the redd count by stream/reach/species/run
  group_by(reach, stream, year, species)  |>
  ...
  bind_rows(clean_battle_redd |> 
              distinct(redd_id, .keep_all = TRUE) |> 
              mutate(redd_count = 1) |> 
              group_by(reach, stream, year, species, run) |> 
  ...
  mutate(species = case_when(species %in% c("Chinook", "CHN") ~ "chinook",
                             species %in% c("UNK", "Unknown", "Salmonid") ~ "unknown",
                             species %in% c("O. mykiss", "SH") ~ "steelhead",
                             species %in% c("P. lamprey", "Lamprey") ~ "pacific lamprey",
                             is.na(species) ~ "not recorded"),
         run = ifelse(is.na(run), "not recorded", run)) |> 
  select(-max_yearly_redd_count) |> 
  ungroup()
...
redd_count_by_year  |> 
  ...
  summarize(annual_redd_count = sum(annual_redd_count, na.rm = T)) |> 
ggplot() + 
  geom_col(aes(x = year, y = annual_redd_count, fill = stream)) + 
  facet_wrap(~stream) +
