...
Substrate sizes were given names and classes following the Wentworth Scale, created by W.C. Krumbein. This scale is what is commonly used in the United States and is as follows. The substrate size provided for redd data was standardized using this scale. When the size range fell into two categories, they were rounded down.
...
lookup <- data.frame("size_range" = unique(all_redd_data$redd_substrate_size),
...
lookup <- lookup %>% left_join(substrate_class)
...
all_redd_data %>%
  ...
  group_by(year) %>%
  mutate(max_redd = max(sum_redd)) %>% glimpse # maximum per year 
```
