...
Due to the format of the data received, two redd datasets were developed. The first dataset includes Feather, Battle, Yuba, and Clear Creek daily data and the second dataset includes Feather, Battle, Clear, and Mill Creek and has a yearly maximum redd counts value.
...
This dataset is the same as `all_redd_data` but includes `max_yearly_redd_count` which is the maximum number of redds counted on a single day per year. This dataset does not include Yuba River because count measurements are not collected for the Yuba.
...
| latitude                 | B, C, F            | Latitude of redd location                                             |
| longitude                | B, C, F            | Longitude of redd location                                            |
...
| velocity                 | B, C, F            | measured stream velocity (ft/s)                                       |
| depth_m                  | F                  | Depth of water (meters)                                               |
| max_yearly_redd_count    | B, C, F, M         | maximum number of redds counted on a day in a unique reach, per year  |
...
-   `num_of_fish_on_redd` - was not included in other redd datasets and deemed unnecessary

```{r warning=FALSE}

yuba_river_raw <- read_csv('../../data/redd-carcass-holding/yuba_redd.csv') |> 
  glimpse()
...
pre_redd_substrate_size_lookup <- all_redd_data %>% 
  ...
  distinct() %>%
  rename(pre_redd_substrate_class = redd_substrate_class) %>% glimpse
...
*TODO: we need to look at how sampling rate/frequency can play into decay rate and potential double counting. The sampling frequency for Battle and Clear seem to be similar while Feather is much more frequent.*

```{r}

# TODO: incorporate decay rate? If 
...
write_csv(all_redd_data, "../../data/redd-carcass-holding/standard_all_redd_data.csv")
write_csv(redd_count_by_year, "../../data/redd-carcass-holding/standard_redd_data_by_year.csv")
