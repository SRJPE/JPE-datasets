...
|:---------------------------|:------------------|:---------------------------------------------------------------------------------------------------------|
|stream                      |F, M, Y, C, B      |Stream associated with redd data collected                                                                |
|year                        |F, M, Y, C, B      |Calendar year sampling occured                                                                            |
|reach                       |F, M, C, B         |Reach sampled                                                                                             |
|species                     |F, M, Y, B, C      |Species associated with redd sampled                                                                      |
|run                         |F, M, Y, C, B      |Run of fish associated with redd sampled                                                                  |
|annual_redd_count           |F, M, Y, C, B      |Number of redds counted in a year by reach, stream, species, and run                                      |
|annual_summarization_method |F, M, Y, C, B      |Summarization method used to get the annual number: either summed by redd id or maximum yearly redd count |
...
redd_non_battle_clear <- all_redd_data |>
  filter(!stream %in% c("battle creek", "clear creek")) |> 
  group_by(stream, reach, date, species, run) |> 
  summarize(daily_reach_count = sum(redd_count, na.rm = TRUE)) |> 
  ungroup() |> 
  group_by(year = year(date), stream, reach, species, run) |> 
  summarize(max_daily_reach_count = max(daily_reach_count, na.rm = TRUE)) |> 
  ungroup() |> 
  group_by(stream, year, reach, species, run) |> 
  summarize(annual_redd_count = sum(max_daily_reach_count, na.rm = TRUE)) |> 
  ungroup() |> 
  glimpse()
  
# for streams with redd id, just sum redd count (each redd id is only counted once)
battle_clear_redd <- all_redd_data |>
  filter(stream %in% c("battle creek", "clear creek")) |> 
  group_by(year = year(date), stream, reach, species, run) |> 
  summarize(annual_redd_count = sum(redd_count, na.rm = TRUE)) |> 
  ungroup()

redd_count_by_year <- bind_rows(redd_non_battle_clear, battle_clear_redd) |>
  mutate(annual_summarization_method = ifelse(stream %in% c("mill creek", "yuba river", "feather river"), "maximum yearly redd count", "summed by redd id"))
```
