...
| origin | B | if released fish are natural or hatchery fish |
...
battle_released <- battle_mark_recap %>% 
  select(location, site, release_id, release_date, release_time, number_released, median_fork_length_released, 
...
```{r}
filter(combined_release, location %in% c("battle creek", "cear creek")) %>% pull(site) %>% unique()
```
...
combined_release %>% ggplot() + 
  geom_point(aes(x = release_date, y = location, color = location), alpha = .1) + 
...
```{r}
combined_release %>% ggplot() + 
  geom_point(aes(x = release_time, y = location, color = location), alpha = .1) + 
  scale_color_manual(values = color_pal) +
  theme_minimal() + 
  scale_x_time() +
  theme(legend.position = "bottom", 
...
```{r}
combined_release %>% ggplot() + 
  geom_histogram(aes(x = number_released, fill = location), alpha = .5) + 
...
```{r}
combined_release %>% 
  ggplot(aes(x = median_fork_length_released, fill = location)) + 
...
combined_release %>% 
  ...
  ggplot(aes(x = median_fork_length_released, y = year, fill = location)) + 
  geom_boxplot() + 
  scale_fill_manual(values = color_pal) +
  theme_minimal() +
...
```{r}
filter(combined_release, location %in% c("sacramento river", "feather river")) %>% pull(day_or_night_release) %>% unique()
```
...
combined_release %>% 
  ggplot() + 
  geom_histogram(aes(x = days_held_post_mark, fill = location), alpha = .5, binwidth = 1) +
  scale_fill_manual(values = color_pal) +
  theme_minimal() + 
  theme(legend.position = "bottom", 
        text = element_text(size = 18),
...
combined_release %>% 
  ggplot() + 
  geom_histogram(aes(x = flow_at_release, fill = location), alpha = .5, binwidth = 25) +
  scale_fill_manual(values = color_pal) +
  theme_minimal() + 
  theme(legend.position = "bottom", 
        text = element_text(size = 18),
...
combined_release %>% 
  ggplot() + 
  geom_histogram(aes(x = temperature_at_release, fill = location), alpha = .5, binwidth = 2.5) +
  scale_fill_manual(values = color_pal) +
  theme_minimal() + 
  theme(legend.position = "bottom", 
        text = element_text(size = 18),
...
combined_release %>% 
  ggplot() + 
  geom_histogram(aes(x = turbidity_at_release, fill = location), alpha = .5, binwidth = 2) +
  scale_fill_manual(values = color_pal) +
  theme_minimal() + 
  theme(legend.position = "bottom", 
        text = element_text(size = 18),
...
```
### origin   
Only battle creek provides origin
```{r}
table(combined_release$origin)
```


```{r}
filter(combined_release, location %in% c("sacramento river", "feather river")) %>% pull(origin) %>% unique()
```
...
combined_recapture %>% ggplot() + 
  geom_point(aes(x = recaptured_date, y = location, color = location), alpha = .1) + 
  scale_color_manual(values = color_pal) +
  theme_minimal() + 
  theme(legend.position = "bottom", 
        text = element_text(size = 18),
...
combined_recapture %>% 
  ...
  ggplot() + 
  geom_histogram(aes(x = number_recaptured, fill = location), alpha = .5, binwidth = 10, position = "identity") +
  scale_fill_manual(values = color_pal) +
  theme_minimal() + 
  theme(legend.position = "bottom", 
        text = element_text(size = 18),
...
```{r}
combined_recapture %>% 
  ggplot(aes(x = median_fork_length_recaptured, fill = location)) + 
...
combined_recapture %>% 
  ...
  ggplot(aes(x = median_fork_length_recaptured, y = year, fill = location)) + 
  geom_boxplot() + 
  scale_fill_manual(values = color_pal) +
  theme_minimal() +
...
```{r}
filter(combined_recapture, location %in% c("sacramento river", "feather river")) %>% pull(cone_status) %>% unique()
```
...
summarized_mark_recapture <- combined_release %>% 
  left_join(combined_recapture, by = c("release_id" = "release_id", "location" = "location")) %>%
  group_by(location, release_id, release_date, number_released) %>%
...
summarized_mark_recapture %>% 
  filter(efficiency < 1) %>% 
  ggplot() + 
  geom_histogram(aes(x = efficiency, fill = location), alpha = .5, position = "identity") + 
...
```{r}
summarized_mark_recapture %>% 
  filter(efficiency < 1, location %in% c("battle creek", "clear creek")) %>%  # one efficiency outlier (TODO check and remove)
  ggplot() + 
  geom_point(aes(x = flow_at_release, y = efficiency, color = location), alpha = .5) + 
  facet_wrap(~location) +
  scale_color_manual(values = color_pal) +
  theme_minimal() + 
  theme(legend.position = "bottom", 
...
summarized_mark_recapture %>% 
  filter(efficiency < 1, location %in% c("battle creek", "clear creek")) %>% # one efficiency outlier (TODO check and remove)
  ggplot() + 
  geom_point(aes(x = temperature_at_release, y = efficiency, color = location), alpha = .5) + 
  facet_wrap(~location) +
  scale_color_manual(values = color_pal) +
  theme_minimal() + 
  theme(legend.position = "bottom", 
...
```{r}
summarized_mark_recapture %>% 
  filter(efficiency < 1, location %in% c("battle creek", "clear creek")) %>%
  ggplot() + 
  geom_point(aes(x = turbidity_at_release, y = efficiency, color = location), alpha = .5) + 
  facet_wrap(~location) +
  scale_color_manual(values = color_pal) +
  theme_minimal() + 
  theme(legend.position = "bottom", 
