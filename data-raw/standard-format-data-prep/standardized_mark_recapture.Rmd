...
Kights Landing does provide some of this data but it is very sparse, so was not included here since we are standardizing historical data. For future data we would want to be able to have a separate table that includes individual release fork lengths for every fish released. 
...
```{r}
summarized_mark_recapture %>% 
  filter(efficiency < 1, stream %in% c("battle creek", "clear creek")) %>%
  ggplot() + 
