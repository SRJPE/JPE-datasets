...
feather_released <- feather_mark_recap %>% 
  select(stream, release_id, site, release_date, release_time, number_released, median_fork_length_released) %>% 
  distinct() %>% 
  glimpse
...
knights_landing_released <- knights_landing_mark_recap %>% 
  select(stream, release_id, site, release_date, number_released, median_fork_length_released) %>% 
  distinct() %>% 
  glimpse
...
combined_release %>% 
  ...
  theme(legend.position = "bottom", 
        ...
        axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) 
```
### origin/adipose_clipped   
Only battle creek provides origin
We converted origin to adipose clipped to be consistant with other datasets
```{r}
table(combined_release$origin)

combined_release <- combined_release %>% 
  mutate(adipose_clipped = case_when(origin == "hatchery" ~ TRUE, 
                                     origin == "natural" ~ FALSE)) %>%
  select(-origin) 

table(combined_release$adipose_clipped)
```


```{r}
filter(combined_release, stream %in% c("sacramento river", "feather river", "clear creek")) %>% pull(adipose_clipped) %>% unique()
```
