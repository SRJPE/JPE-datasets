...
new_deer_environmental <- deer_mill_trap_raw |> 
  filter(stream == "deer creek", date > max(deer_env$date)) |> 
  mutate(site = stream,
         subsite = site) |> 
  select(date, stream, site, subsite, turbidity, weather, temperature = water_temperature) %>%
  distinct() %>%
  mutate_if(is.numeric, as.character) %>% 
  pivot_longer(cols = c("turbidity", "weather", "temperature"), values_to = "text", names_to = "parameter") %>% 
  mutate(value = as.numeric(text),
         text = ifelse(!is.na(value), NA_character_, text)) %>% 
  glimpse()

deer_env_clean_combined <- bind_rows(deer_env,
                                      new_deer_environmental)
```
...
```{r}
new_mill_environmental <- deer_mill_trap_raw |> 
  filter(stream == "mill creek", date > max(mill_env$date)) |> 
  mutate(site = stream,
         subsite = site) |> 
  select(date, stream, site, subsite, turbidity, weather, temperature = water_temperature) %>%
  distinct() %>%
...
mill_env_clean_combined <- bind_rows(mill_env,
                                      new_mill_environmental)
```
### Yuba River
...
```{r}
combined_env <- bind_rows(battle_clear_env, butte_env, deer_env_clean_combined,
                          feather_env, knights_env, tisdale_env, 
                          mill_env_clean_combined, yuba_env, rbdd_environmental) %>% 
