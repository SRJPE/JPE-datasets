...
std_temp <- read.csv(here::here("data-raw", "standard-format-data-prep", "temp_data", "standard_temperature.csv")) 

#Changing "date" field into date format
std_temp$date <- as.Date(std_temp$date)
glimpse(std_temp)
...
table(std_temp$month)

spawning_months <- c("08", "09", "10", "11", "12")

spawn_temps <- std_temp |> 
  filter(month%in%spawning_months)

#filter(date!= as.Date("2000-12-09"))  #removes one day with NA max temp 

#Making sure we only have the months we need - check why only months 10,11,12 appear
table(spawn_temps$month)

#Delete "month field" 
spawn_temps |> select(-month)

#Calculating Butte 7DADM
butte_7dadm <-  spawn_temps |> 
  filter(stream == "butte creek") |> 
  group_by(site, subsite) |> 
  mutate(rolling_7DADM = zoo::rollmean(max_daily_temp_c, k = 7, align = "center", fill = NA)) #Look into NA values


glimpse(butte_7dadm)

#Filtering days when 7DADM is below 13C
butte_7dadm_below13 <- butte_7dadm |> 
filter( rolling_7DADM < 13)
glimpse(butte_7dadm_below13)

#plotting - 7dadm below 13 
ggplot(butte_7dadm_below13, aes(x = date, y = rolling_7DADM))+
