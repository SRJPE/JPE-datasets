...
# token <- Sys.getenv("token") #noaa cdo api token saved

get_temp <- function(start_date, end_date, noaa_token = mytoken) {
...
yuba_air_temps = do.call("rbind", temp_pulls)
...
yuba_air_gage2 <- yuba_air_gage2$data %>% 
  select(date,value) %>% 
  mutate(date = as.Date(date))

yuba_air_gage3 <- rnoaa::ncdc(datasetid = 'GSOM', stationid = 'GHCND:USR0000CFOR', 
                         datatypeid = 'TAVG', startdate = '2013-01-01', 
                         enddate = '2022-06-29', token = "IsRvscvYWFkXtEhLZAEIEyUlMXMCNIch", limit = 130)

yuba_air_gage3 <- tisdale_air_gage3$data %>% 
  select(date,value) %>% 
  mutate(date = as.Date(date))

# Combine Gage ranges spans
rbind(as.data.frame(yuba_air_gage1), as.data.frame(yuba_air_gage2))
yuba_air_gage12 = rbind(as.data.frame(yuba_air_gage1), as.data.frame(yuba_air_gage2))
yuba_air_gage123 = rbind(yuba_air_gage12, as.data.frame(yuba_air_gage3))
yuba_air_gage123
```
...
tisdale_air_gage rnoaa::ncdc(datasetid = 'GSOM', stationid = 'GHCND:USC00R0000CCOUdat
                         atypeid = 'TAVG', '»¿rtdate = '1991995-01', end
