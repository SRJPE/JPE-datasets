...
clear_holding_clean <- clear_holding %>%
  select(-c(comments, picket_weir_location_rm)) %>%
...
### Variables removed
- comments, picket_weir_location_rm
- picket_weir_location_rm is the same value for the entire dataset; it will be added to the metadata
...
# butte_data_dictionary <- tibble(variables = colnames(butte_holding_02), description = c("Date of Sampling", "Reach within stream", "Name of survey person", "Fish count", "Comments why count is NA"), percent_na = round(percent_na$value*100))
...
holding_catch_data_dictionary <- tibble(variable_name = colnames(combined_holding), description = c("Date survey was conducted", "Reach number within stream", "River mile number", "Number of fish observed: Butte creek data has multiple observations from different personnels per day - to prevent double counting, the average of each day at each reach is used as the daily fish count", "Number of jacks observed", "Latitude measurement of adult holding", "Longitude measurement of adult holding", "stream name","Year survey was conducted", "Reason survey was conducted", "Fish above or below Picket Weir"), encoding = c("YYYY-MM-DD", NA,NA,NA,NA,NA,NA,NA, "YYYY", "august index, spawning", "above, below" ), collected = c("All but Deer", "All","Battle and Clear", "All", "Battle and Clear", "Battle and Clear", "Battle and Clear", "All", "All","Clear", "Clear"))
```
```{r, data_dictionary}
...
Average count per month. This chart shows Battle, Butte and Clear as Deer does not have monthly count.
...
Rivermile distribution for Battle and Clear
