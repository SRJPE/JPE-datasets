...
battle_upstream_passage <- battle_video |> 
  bind_rows(battle_spawning_building |> 
              select(date, stream, time, count, sex, confidence_in_sex, fork_length, adipose_clipped, status) |> 
              mutate(dead = ifelse(status %in% c("sacraficed", "mortality", "dead in pond"), T, F)) |> # label mortalities
