...
mill_upstream_passage <- read_csv(here::here("data", "adult-upstream-passage-monitoring", "mill_adult_upstream.csv")) |> 
  filter(!is.na(date)) |> # removes 2 NA rows
  rename(count = passage_estimate,
         passage_direction = direction, # confirm with matt johnson that this is a count not a modeled passage
         jack_size = jack,
         viewing_condition = view_adjust) |> 
