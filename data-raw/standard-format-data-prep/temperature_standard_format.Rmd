...
4.  If no, build a regression model with another similar stream in order to fill in data gaps.
...
-   TODO update with data through 2023
...
-   The CDEC gage `BCK` was used to aggregate temperature for sites
    `adams dam` and `okie dam` from `1998-09-16` to `2022-06-30`. USGS data was used to establish threshold to identify min and max temperatures. Outliers were interpolated using linear approximation methods
...
-   Deer Creek data was aggregated from CDEC's site `DCV` from
    `1998-10-01` to `2022-07-13`. USGS data was used to identify outliers. Outliers were interpolated using linear approximation.
...
- GRL will represent the High Flow Channel (HFC) and FRA will represent the Low Flow Channel (LFC). Data collected at the RST will be appended to these datasets. Eye Riffle site will be used to represent all LFC sites and Herringer Riffle will be used to represent all HFC sites as these datasets are the most extensive.
...
feather_hfc_format_combined <- bind_rows(filter(feather_rst_hfc, year(date) > 2007), # rst filter so does not overlap with gage data
                        feather_hfc_format) %>% 
  group_by(stream, date, mean_temp_degC, max_temp_degC, min_temp_degC, gage_agency, gage_number, site_group) |> 
  distinct() |> glimpse()
ggplot() +
...
test_predict_df <- test |> 
  mutate(feather_predicted = test_predict,
         percent_off = (feather_temp - feather_predicted)/feather_temp)
...
sac_format_all <- full_join(sac_format, sac_sites) |> 
  mutate(mean_temp_degC = case_when(mean_temp_degC < 0.5 ~ NA_real_,
                               T ~ mean_temp_degC),
         mean_temp_degC = as.numeric(mean_temp_degC),
         mean_temp_degC = zoo::na.approx(mean_temp_degC, na.rm = F))
...
YR7 CDEC gage is used, however, this only contains data from 2020 onwards. Temperature data collected along with RST data is used to fill the gap prior to 2020. Additional work needs to be done to fill data gaps.
...
yuba_format_all <- full_join(yuba_format, yuba_sites)

yuba_format_all %>% ggplot() + 
  geom_line(aes(x = date, y = mean_temp_degC, color = site)) + 
  # geom_point(aes(x = date, y = mean_temp_degC), size = .001) +
  theme_minimal() 
...
```{r}
yuba_combined <- bind_rows(yuba_format_all, yuba_rst_format)

ggplot() +
  geom_line(data = yuba_combined, aes(x = date, y = mean_temp_degC, color = site)) 
```
