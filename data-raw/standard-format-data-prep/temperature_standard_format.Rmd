...
-   Feather River (source: CDEC)
-   Battle Creek (source: USFWS)
-   Butte Creek (source: CDEC)
-   Clear Creek (source: USFWS)
-   Deer Creek (source: CDEC)
-   Mill Creek (source: CDEC)
-   Sacramento River (source: USGS)
-   Yuba River (source: CDEC)
...
| value             | all               | Temperature value in degress Celsius                |
...
feather_format_all_hfc <- full_join(feather_hfc_format, feather_hfc_sites)

summary(feather_format_all_hfc$date)

feather_format_all_hfc %>% ggplot() + 
  geom_line(aes(x = date, y = mean_temp_degC, color = site)) + 
  # geom_point(aes(x = date, y = mean_temp_degC), size = .001) +
  theme_minimal() 
...
feather_format_all_lfc <- full_join(feather_lfc_format, feather_lfc_sites)

summary(feather_format_all_lfc$date)

feather_format_all_lfc %>% ggplot() + 
  geom_line(aes(x = date, y = mean_temp_degC, color = site)) + 
  # geom_point(aes(x = date, y = mean_temp_degC), size = .001) +
  theme_minimal() 
```
...
```{r}
WLK_USGS <- readRDS(here::here("data-raw", "standard-format-data-prep","temp_data", "sac_temp.rds"))
