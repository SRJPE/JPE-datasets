...
Completeness of data 2001-2018: 
```{r, echo=FALSE}
#From Loggers
...
Note that outliers were removed by setting temperature range from 0-37 degrees C

```{r, echo=FALSE}
# Filter clean data to show only numeric variables 
#checking temporal coverage across deer sites from logger
mill_plot_logger <- ggplot(combined_table_mill, aes(x = date, y = temperature, color = "")) +
...
## Plot showing CDEC (MLM station) mean temperatures
...
ggplot() +
  ...
  labs(title = "Monthly Mean Temperatures from CDEC station MLM",
...
## Plot comparing CDEC MLM mean temperature and logger data from Mill 8-0 Mouth
...
Recommendation: use CDEC data, this might not be a good dataset to rely on, it seems to overlap a little with CDEC anyways
...
## Plot comparing CDEC MLM mean temperature and logger data from Mill 8-1 VIDEO Station

Comments: 

  - Temperature from this logger could have inaccuracies. 
  - During the months of Jul, Aug, Sept, the plot shows no data ranges, this suggests this logger data either doesn't have complete data throughout the months, or something the temperatures recorded during those months is way too similar

Recommendation: 

  - Should not use this data since ranges seem off
...
Recommendation: 
  ...
  - Data can be used in addition to CDEC, since the locations is higher up the stream and seems reasonable
...
## Plot comparing CDEC MLM mean temperature and logger data from Mill 8-3 Rancheria 

Comments:

  - The range of mean temperatures throughout the months is a bid wider than CDEC (and other sites from logger)
  - Winter months don't vary so much from CDEC, but fall and summer do. 
  - The mean temperature is lower during those seasons with a high of ~18 degrees C during summer
...
## Plot comparing CDEC MLM mean temperature and logger data from Mill 8-4 Black Rock
...
## Plot comparing CDEC MLM mean temperature and logger data from Mill 8-5 Sooner

Comments: 

  - Temperature relatively consistent throughout January and February. Ranging from ~4-6 degrees C. 
  - Summer months are significantly cooler than CDECs data, and this could be do to the location difference. 
  - Logger temperatures go only as high as ~16 degrees C 
  
Recommendation: 

  - Use this logger data since coverage is different than CDEC and does not seem to have unreasonable overall temperature values
...
## Plot comparing CDEC MLM mean temperature and logger data from Mill 8-6 Canyon Camp

Comments: 

  - Temperature ranges in most months are significantly small, varying ~ less than a degrees Celsius. Specially for all months except May, June, and July
  - On the other hand, June temperatures have a significantly bigger range ~8 to 12.5 degrees Celsius
  
Recommendation: 

  - Since it is only one month that has that big of a range the recommendation would be to not rely on that logger, at least for that month
...
## Plot comparing CDEC MLM mean temperature and logger data from Mill 8-7 Hole in Ground

Comments: 

  - This temperature ranges are relatively big throughout. This location is surrounded by big mountain ranges so that could be the reason of this temperature fluctuation
...
## Plot comparing CDEC MLM mean temperature and logger data from Mill 8-8 Little Hole in Ground
...
**NA and Unknown Values**
  ...
  - No "NA" values found throughout the data
