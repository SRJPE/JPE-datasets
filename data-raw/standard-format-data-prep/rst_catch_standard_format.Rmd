...
butte_rst_clean <- butte_rst %>%
  select(date, station, dead, count, fork_length, weight, mark_code, lifestage, time) %>%
...
# TODO use CAMP to find run_method
...
yuba_rst_clean<- yuba_rst %>%
  select(date, time, fork_length, weight, lifestage, count, location, run) %>%
  group_by(date, fork_length, weight, lifestage, run) %>%
  summarize(count = sum(count, na.rm = T)) %>%
...
tisdale_rst_clean <- tisdale_rst %>%
  ...
  select(-c(fish_processed, at_capture_run, final_run, release_id, mark_type, mark_position, mark_color, trap_visit_comment)) %>%
  rename(fork_length = fork_length_mm,
         origin = rearing,
         lifestage = life_stage,
         dead = mortality) %>%
  group_by(date, species, fork_length, weight, lifestage, dead, run, origin, random) %>%
  summarize(count = sum(count, na.rm = T)) %>%
...
combined_rst_raw <- bind_rows(battle_rst_clean,
                          butte_rst_clean,
                          clear_rst_clean,
                          deer_rst_clean,
                          feather_rst_clean,
...
rst_catch_data_dictionary <- tibble(variable_name = colnames(combined_rst),
                                    description = c("Date trap checked. Some locations also have a time component.",
                                                    ...
                                                    "Site of trap within tributary. Some locations do not have multiples sites in which tributary = site. Site names are specific to the tributary.",
                                                    "Describes if the fish is natural or hatchery origin. In some cases this information may be unknown.",
                                                    "Weight in grams.",
                                                    "Time trap checked.",
                                                    "Species of fish. All were filtered to chinook.",
                                                    "Method used to designate run. NA if count is 0.",
                                                    "Describes if fish were caught randomly (T/F)"),
                                    
                                    encoding = c(NA,
                                                 "late fall, spring, fall, winter, not recorded, unknown, outlier salmon",
                                                 NA,
                                                 "smolt, fry, yolk sac fry, not recorded, parr, silvery parr, adult, unknown, yearling",
                                                 NA,
                                                 "TRUE, FALSE",
                                                 "TRUE, FALSE",
                                                 "Battle Creek, Butte Creek, Clear Creek, Deer Creek, Feather River, Mill Creek, Yuba River, Lower Sacramento - Knights Landing, Lower Sacramento - Tisdale",
                                                 "Battle Creek, Okie Dam 1, Adams Dam, Okie Dam 2, LCC, UCC, Deer Creek, Eye Riffle, Live Oak, Herringer Riffle, Steep Riffle, Sunset Pumps, Shawns Beach, Gateway Riffle, Mill Creek, Yuba River, Lower Sacramento - Knights Landing, Lower Sacramento - Tisdale",
                                                 "natural, not recorded, hatchery, unknown",
                                                 NA,
                                                 NA,
                                                 "chinook salmon",
                                    ...
                                    `not collected` = c(NA,
                                                       ...
                                                       "Deer, Mill",
                                                       NA,
                                                       "Deer, Feather, Mill, Yuba, Knights",
                                                       "Butte, Deer, Feather, Mill, Yuba, Knights, Tisdale",
                                                       NA,
                                                       NA,
                                                       NA,
                                                       "Battle, Clear, Feather, Mill",
                                                       "Battle, Clear, Deer, Feather, Mill, Yuba, Tisdale",
                                                       NA,
                                                       "Battle, Butte, Clear, Deer, Feather, Mill, Yuba, Tisdale",
...
combined_rst %>%
  mutate(year = year(date)) %>%
  group_by(tributary, year, lifestage) %>%
  summarize(count = sum(count, na.rm = T)) %>%
  left_join(total_count) %>%
  mutate(percent = (count/total)*100) %>%
...
combined_rst %>%
  filter(tributary %in% c("Battle Creek", "Clear Creek")) %>%
  group_by(tributary, date, interpolated) %>%
  summarize(count = sum(count, na.rm = T)) %>%
...
combined_rst %>%
  mutate(year = year(date)) %>%
  group_by(tributary, year, origin) %>%
  summarize(count = sum(count, na.rm = T)) %>%
  left_join(total_count) %>%
  mutate(percent = (count/total)*100) %>%
...
combined_rst %>%
  mutate(year = year(date)) %>%
  group_by(tributary, year, run_method) %>%
  summarize(count = sum(count, na.rm = T)) %>%
  left_join(total_count) %>%
  mutate(percent = (count/total)*100) %>%
...
Only Tisdale recorded this information. There are very few cases where random is not true.
