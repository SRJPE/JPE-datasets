...
# Knights landing data pulled from camp (knights_camp_catch_data.Rmd)
gcs_get_object(object_name = "rst/lower-sac-river/data-raw/knights-landing/knights_catch_camp.csv",
               bucket = gcs_get_global_bucket(),
               saveToDisk = "data/rst/knights_catch.csv",
               overwrite = TRUE)

knights_landing_rst <- read_csv("data/rst/knights_catch.csv")
knights_landing_rst_excel <- read_csv("data/rst/knights_rst_clean.csv")
...
battle_data_dictionary <- read_rds("data-raw/qc-markdowns/rst/data-dictionaries/battle_rst_data_dictionary.rds")
...
butte_data_dictionary <- read_rds("data-raw/qc-markdowns/rst/data-dictionaries/butte_rst_data_dictionary.rds")
...
clear_data_dictionary <- read_rds("data-raw/qc-markdowns/rst/data-dictionaries/clear_rst_data_dictionary.rds")
...
deer_data_dictionary <- read_rds("data-raw/qc-markdowns/rst/data-dictionaries/deer_rst_data_dictionary.rds")
...
feather_data_dictionary <- read_rds("data-raw/qc-markdowns/rst/data-dictionaries/feather_rst_data_dictionary.rds")
...
mill_data_dictionary <- read_rds("data-raw/qc-markdowns/rst/data-dictionaries/mill_rst_data_dictionary.rds")
...
yuba_data_dictionary <- read_rds("data-raw/qc-markdowns/rst/data-dictionaries/yuba_rst_data_dictionary.rds")
...
knights_rst_clean_camp <- knights_landing_rst %>%
  select(-c(catchRawID, trapVisitID, lifeStageCAMPID, visitTime2, visitType, releaseID)) %>%
  rename(count = n,
         date = visitTime,
...
knights_rst_clean_pre2006 <- knights_landing_rst_excel %>% 
  ...
  mutate(adipose_clipped = case_when(adipose_clipped == T ~ "Natural",
                                     adipose_clipped == F ~ "Hatchery",
                                     T ~ "Unknown"),
         count = ifelse(is.na(count), 0, count),
         # this likely happened from reshaping of data
         remove = case_when(is.na(run) & species != "Steelhead" ~ "remove",
                            species == "Steelhead" ~ "remove",
                            T ~ "keep")) %>% 
  filter(remove != "remove") %>% 
  select(-remove) %>% 
  group_by(date, species, run, lifestage, adipose_clipped) %>% 
...
tisdale_data_dictionary <- read_rds("data-raw/qc-markdowns/rst/data-dictionaries/lower_sac_tisdale_rst_data_dictionary.rds")
