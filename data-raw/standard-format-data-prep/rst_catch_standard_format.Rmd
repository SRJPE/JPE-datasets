...
butte_rst_clean <- butte_rst %>%
  ...
  mutate(location = "butte creek",
         ...
         run_method = "unknown") %>%
  group_by(date, site, dead, fork_length, weight, origin, lifestage, location, run, run_method) %>%
  summarize(count = sum(count, na.rm = T))
```
...
yuba_rst_clean<- yuba_rst %>%
  select(date, fork_length, weight, lifestage, count, run) %>%
...
knights_rst_clean <- knights_landing_rst %>%
  ...
  rename(count = n,
         ...
         lifestage = lifeStage) %>%
  group_by(fork_length, weight, species, run, run_method, lifestage, origin, date) %>%
  summarize(count = sum(count, na.rm = T)) %>%
...
tisdale_rst_clean <- tisdale_rst %>%
  ...
  rename(fork_length = fork_length_mm,
         ...
         dead = mortality) %>%
  group_by(date, species, fork_length, weight, lifestage, dead, run, origin, random, mark_type) %>%
  summarize(count = sum(count, na.rm = T)) %>%
  mutate(location = "sacramento river",
         site = "tisdale",
         origin = tolower(origin),
...
rst_catch_data_dictionary <- tibble(variable_name = colnames(combined_rst),
                                    description = c("Date trap checked. Some locations also have a time component.",
                                                    ...
                                                    "Site of trap within location. Some locations do not have multiples sites in which location = site. Site names are specific to the tributary.",
                                                    "Describes if the fish is natural or hatchery origin. In some cases this information may be unknown.",
                                                    "Weight in grams.",
                                                    "Subsite location of trap",
...
combined_rst %>%
  mutate(year = year(date)) %>%
  group_by(location, year, lifestage) %>%
  summarize(count = sum(count, na.rm = T)) %>%
  left_join(total_count) %>%
  mutate(percent = (count/total)*100) %>%
...
combined_rst %>%
  filter(location %in% c("battle creek", "clear creek")) %>%
  group_by(location, date, interpolated) %>%
  summarize(count = sum(count, na.rm = T)) %>%
...
combined_rst %>%
  mutate(year = year(date)) %>%
  group_by(location, year, run_method) %>%
  summarize(count = sum(count, na.rm = T)) %>%
  left_join(total_count) %>%
  mutate(percent = (count/total)*100) %>%
