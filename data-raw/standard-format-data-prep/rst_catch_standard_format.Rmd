...
Data that are stored in CAMP are now on EDI. We pull from EDI rather than using CAMP db files.
...
butte_catch_edi <- catch_edi |> 
  ...
  filter(date <= as_date("2022-06-30") & date > as_date("2015-06-03")) |> 
  select(date, stream, site, subsite, site_group, count, run, lifestage, adipose_clipped,
...
```{r, combine_butte}
# bind rows
butte_rst_clean <- bind_rows(butte_rst_clean_pre_2015, butte_catch_edi) %>% glimpse()
```
