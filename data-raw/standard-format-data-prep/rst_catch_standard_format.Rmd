...
butte_rst_clean <- butte_rst %>%
  ...
  mutate(stream = "butte creek",
         ...
         species = "chinook salmon") %>%
  group_by(date, site, dead, fork_length, weight, adipose_clipped, lifestage, stream, run, run_method, species) %>%
  summarize(count = sum(count, na.rm = T)) 
...
feather_camp_test <- feather_rst_camp %>% 
  select(-c(catchRawID, trapVisitID, releaseID, visitTime2, markType, color, bodyPart)) %>%
  #combine atcapture and final run
  mutate(run = ifelse(!is.na(final_run) & final_run != "Not recorded", final_run, run),
         runMethod = ifelse(!is.na(final_run_method), final_run_method, runMethod)) %>%
  select(-c(final_run, final_run_method)) %>%
...
feather_rst_clean <- feather_rst_camp %>%
  ...
  rename(fork_length = forkLength,
         ...
         site = siteName) %>%
  group_by(date, species, fork_length, weight, lifestage, dead, run, adipose_clipped, run_method, site) %>%
  summarize(count = sum(n, na.rm = T)) %>%
...
tisdale_rst_clean <- tisdale_rst_camp %>%
  # removing existing mark for now. will add all those in later
  select(-c(catchRawID, trapVisitID, releaseID, visitTime2, markType, color, bodyPart, visitType)) %>%
  # combine atcapture and final run
  mutate(run = ifelse(!is.na(final_run), final_run, run),
         runMethod = ifelse(!is.na(final_run_method), final_run_method, runMethod)) %>% 
  select(-c(final_run, final_run_method)) %>% 
  rename(fork_length = forkLength,
         ...
         date = visitTime,
         run_method = runMethod) %>%
  group_by(date, species, fork_length, weight, lifestage, dead, run, adipose_clipped, run_method) %>%
  summarize(count = sum(n, na.rm = T)) %>%
