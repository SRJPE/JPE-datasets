...
knights_rst_clean_pre2002 <- knights_landing_pre2002 %>% 
  ...
  rename(subsite = trapid,
         ...
         dead = mort) %>% 
  # filter out marks (these are recaptures except in 1997 when they may be fish caught in trap for first time)
  filter(is.na(mark) | mark == F | (date >= "1996-11-24" & date <= "1997-10-03")) %>% 
