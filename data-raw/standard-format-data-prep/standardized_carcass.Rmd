...
battle_carcass <- read_csv("data/redd-carcass-holding/battle_carcass.csv") |> 
  mutate(stream = "battle creek",
         count = 1, 
         run = NA) |>  # spring run and unknown mixed
...
* all individual-level carcass data from 2019 does not have associated `date` information. These data only have resolution at the `year` level. All other individual-level carcass data tables for years `2014-2020` have `date` information.
...
butte_carcass_2017_2020 <- read_csv("data/redd-carcass-holding/butte_carcass-2017-2020.csv") |> 
  mutate(date = if_else(year == 2019, as.Date("2019-01-01"), date)) |>  # TODO false dates to keep 2019 year data in the final dataset
  select(-year)
...
yuba_carcass <- read_csv("data/redd-carcass-holding/yuba_carcass.csv") |> 
  ...
  mutate(stream = "yuba river",
         run = NA) |> # no documentation or data recorded about run 
  glimpse()
```
