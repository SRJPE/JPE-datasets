...
| release_origin                      | B, F, KL, T           | fish origin (natural, hatchery, mixed, unknown, not recorded, or NA)                                                         |
| release_run                         | T                   | run of fish released
        |
...
| cone_status                   | B, C, T               | if RST cone is fishing half or full                          |
...
feather_released <- feather_mark_recap %>% 
  select(stream, release_id, site, release_date, release_time, number_released, median_fork_length_released, release_origin = fish_origin) %>% 
  rename(date_released = release_date) %>% 
  distinct() %>% 
  glimpse
  
feather_recaptured <- feather_mark_recap %>% 
  select(stream, site, release_id, recaptured_date, number_recaptured, median_fork_length_recaptured) %>% 
  rename(date_recaptured = recaptured_date) %>% 
...
tisdale_released <- read_csv(here::here("data", "rst", "tisdale_releases.csv")) %>% 
  mutate(stream = "sacramento river",
         site = "tisdale",
         release_site = tolower(release_site),
         release_id = ifelse(is.na(release_id), paste0("TIS", row_number()), release_id),
         day_or_night_release = ifelse(light_condition %in% c("Twilight / dusk", "Night, dark", "Night, moonlit"), "night", "day")) %>% 
  rename(release_run = run_released) %>% 
...
tisdale_recaptured <- read_csv(here::here("data", "rst", "tisdale_recaptures.csv")) %>% 
  mutate(stream = "sacramento river",
         site = "tisdale",
         release_id = ifelse(is.na(release_id), paste0("TIS", row_number()), release_id)) %>% 
  group_by(stream, site, release_id, date_recaptured, half_cone) %>% 
  summarize(number_recaptured = sum(number_recaptured),
            median_fork_length = median(fork_length, na.rm = T)) %>%
  mutate(cone_status = ifelse(half_cone == T, "half", "full")) %>% 
  select(-half_cone)
```
...
```{r}
combined_release <- bind_rows(battle_released, clear_released, feather_released, knights_landing_released, tisdale_released) %>% 
  relocate(stream, site, release_id, date_released, release_time, number_released, 
           ...
           release_origin, release_run) %>% 
  mutate(release_origin = tolower(release_origin),
         release_origin = ifelse(is.na(release_origin), "not recorded", release_origin),
         release_run = tolower(release_run)) %>% 
  distinct() %>%glimpse

combined_recapture <- bind_rows(battle_recaptured, clear_recaptured, feather_recaptured, knights_landing_recaptured, tisdale_recaptured) %>% 
  relocate(stream, release_id,
           date_recaptured, number_recaptured, median_fork_length_recaptured, cone_status) %>% glimpse
```
...
```{r}
table(combined_release$release_site)
```

### release_id

Release id are unique identifiers for an efficiency trial on a specific tributary. There are 1794 unique release IDs
...
```{r}
table(combined_release$release_origin, useNA = "ifany")
```

```{r}
filter(combined_release, stream %in% c("sacramento river", "feather river", "clear creek")) %>% pull(release_origin) %>% unique()
```
...
```{r}
table(combined_release$release_run)
```
...
Only battle and clear and tisdale provide a cone_status
...
summarized_mark_recapture <- combined_release %>% 
  left_join(combined_recapture, by = c("release_id" = "release_id", "stream" = "stream", "site" = "site")) %>%
  group_by(stream, site, release_id, date_released, number_released) %>%
...
combined_recapture <- combined_recapture %>% 
  mutate(date_recaptured = as_date(date_recaptured)) %>% glimpse 
