...
feather_released <- read_csv(here::here("data", "rst", "feather_releases.csv")) %>% 
  mutate(site_released = tolower(site_released),
         site_released = case_when(grepl("eye", site_released) ~ "eye riffle",
                          grepl("live", site_released) ~ "live oak",
                          grepl("herringer", site_released) ~ "herringer riffle",
                          grepl("steep", site_released) ~ "steep riffle",
                          grepl("sunset", site_released) ~ "sunset pumps",
                          grepl("gateway", site_released) ~ "gateway riffle",
                          T ~ "not recorded"),
...
knights_recaptured <- read_csv(here::here("data", "rst", "knights_recaptures.csv")) %>% 
  ...
  summarize(number_recaptured = sum(number_recaptured),
            median_fork_length_recaptured = median(fork_length, na.rm = T)) %>% 
  mutate(release_id = as.character(release_id),
         subsite = as.character(subsite)) %>% glimpse()
```
...
```{r}
combined_release <- bind_rows(battle_released, clear_released, feather_released, knights_released, tisdale_released) %>% 
...
combined_recapture <- bind_rows(battle_recaptured, clear_recaptured, feather_recaptured, knights_recaptured, tisdale_recaptured) %>%
