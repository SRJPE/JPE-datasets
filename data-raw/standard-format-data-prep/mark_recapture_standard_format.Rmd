...
butte_released <- read_csv(here::here("data", "rst", "butte_releases.csv")) %>% 
  ...
  # remove the release trial associated with when only the fyke was running
  filter(!(month(date_released) == 10 & year(date_released) == 2021)) |> 
  glimpse()
...
raw <- read_data_entity(packageId = "edi.1504.3", entityId = res$entityId[2])
deer_mill_recapture_raw <- read_csv(file = raw)

raw <- read_data_entity(packageId = "edi.1504.3", entityId = res$entityId[3])
deer_mill_release_raw <- read_csv(file = raw)

deer_mill_release <- deer_mill_release_raw |> 
  mutate(site = stream) |> 
  rename(date_released = release_date,
         median_fork_length_released = mean_fl_released,
         origin_released = release_origin)

deer_mill_recapture <- deer_mill_recapture_raw |> 
  mutate(site = stream,
         subsite = site) |> 
  rename(date_recaptured = recapture_date,
         number_recaptured = total_recaptured,
         median_fork_length_recaptured = mean_fl_recaptured) |> 
  select(-c(min_fl_recaptured, max_fl_recaptured)) |> 
  mutate(date_recaptured = case_when(date_recaptured == "2032-01-24" ~ as.Date("2023-01-24"),
                                     T ~ as.Date(date_recaptured)))
```
...
knights_recaptured <- read_csv(here::here("data", "rst", "knights_recaptures.csv")) %>% 
  ...
  select(-c(date_released, week_released, year_released)) |> 
glimpse()
```
...
```{r}
combined_release <- bind_rows(battle_clear_released, butte_released, feather_released, knights_released, tisdale_released, deer_mill_release) %>% 
...
combined_recapture <- bind_rows(battle_clear_recaptured, butte_recaptured, feather_recaptured, knights_recaptured, tisdale_recaptured, rbdd_recapture, deer_mill_recapture) %>%
