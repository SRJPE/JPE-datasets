...
combined_upstream_passage <- bind_rows(battle_upstream_passage, clear_upstream_passage, 
  ...
  relocate(stream, date, time, count, run, adipose, sex, passage_direction, 
...
combined_upstream_passage <- combined_upstream_passage %>% 
  mutate(run = case_when(run == "SR" ~ "spring",
                         ...
                         run == "FR" ~ "fall", 
                         run == "unknown" ~ "unknown"),
         run = ifelse(stream %in% c("Mill Creek", "Deer Creek"), "spring run", run))
...
combined_upstream_passage %>% 
  ggplot() +
  geom_histogram(aes(x = flow, fill = stream), binwidth = 30, alpha = .5, position = "identity") + 
   scale_fill_manual(values = color_pal) +
  theme_minimal() + 
  theme(legend.position = "bottom", 
        text = element_text(size = 18),
