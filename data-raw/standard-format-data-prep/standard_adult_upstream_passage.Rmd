...
|:------------------|:------------------|:---------------------------------|
| location           | **B, C, D, M, Y**  | which Spring Run JPE stream is the data from                                                           |
| date               | **B, C, D, M, Y**  | date of video footage                                                                                  |
| time               | **B, C, Y**        | time of video footage                                                                                  |
| count              | **B, C, D, M, Y**  | number of fish observed                                                                                |
| adipose            | **B, C, Y**        | if adipose fin is present (TRUE/FALSE)                                                                 |
...
combined_upstream_passage <- bind_rows(battle_upstream_passage, clear_upstream_passage, 
  ...
  relocate(location, date, time, count, run, adipose, sex, passage_direction, 
           viewing_condition, spawning_condition, jack_size, ladder, flow, hours, temperature) %>%
  glimpse
```
...
```{r}
unique(combined_upstream_passage$location)
```
...
combined_upstream_passage %>% ggplot() + 
  geom_point(aes(x = date, y = location, color = location), alpha = .1) +
...
```{r}
combined_upstream_passage %>% ggplot() + 
  geom_point(aes(x = time, y = location, color = location), alpha = .1) +
  scale_color_manual(values = color_pal) +
  theme_minimal() + 
  theme(legend.position = "bottom", 
        text = element_text(size = 18),
...
daily %>% ggplot() + 
  geom_boxplot(aes(x = count,y = location,  color = location), 
                 alpha = .5, binwidth  = 1) + 
  scale_color_manual(values = color_pal) +
  theme_minimal() + 
  theme(legend.position = "bottom", 
...
combined_upstream_passage <- combined_upstream_passage %>% 
  mutate(run = case_when(run == "SR" ~ "spring",
                         ...
                         run == "FR" ~ "fall", 
                         run == "unknown" ~ "unknown"),
         run = ifelse(location %in% c("Mill Creek", "Deer Creek"), "spring", run))
...
Change adipose field to origin field

Look at unique values for adipose

```{r}
unique(combined_upstream_passage$adipose)
```

Change clipped to absent for adipose and reassign as origin: hatchery, natural, or unknown

```{r}
combined_upstream_passage <- combined_upstream_passage %>% 
  mutate(adipose = ifelse(adipose == "clipped", "absent", adipose),
         origin = case_when(adipose == "absent" ~ "hatchery", 
                            adipose == "present" ~ "natural", 
                            adipose == "unknown" ~ "unknown")) %>%
  select(-adipose)

combined_upstream_passage %>% 
  group_by(location) %>%
  summarise(origin = unique(origin) %>% base::paste(collapse = ', ')) 
```
...
combined_upstream_passage %>% 
  ggplot() +
  geom_histogram(aes(x = hours, fill = location), binwidth = 1) + 
...
combined_upstream_passage %>% 
  ggplot() +
  geom_histogram(aes(x = flow, fill = location), 
...
```{r}
combined_upstream_passage %>% 
  filter(location %in% c("Mill Creek", "Deer Creek")) %>%
  ggplot() +
  geom_line(aes(x = date, y = flow, color = location), size = 1) + 
  scale_color_manual(values = color_pal) +
  theme_minimal() + 
  theme(legend.position = "bottom", 
        text = element_text(size = 18),
...
combined_upstream_passage %>% 
  ggplot() +
  geom_histogram(aes(x = temperature, fill = location), binwidth = 1, alpha = .5, position = "identity") + 
   scale_fill_manual(values = color_pal) +
  theme_minimal() + 
  theme(legend.position = "bottom", 
        text = element_text(size = 18),
...
combined_upstream_passage %>% 
  filter(location %in% c("Mill Creek", "Deer Creek")) %>%
  ggplot() +
  geom_line(aes(x = date, y = temperature, color = location), size = 1) + 
  scale_color_manual(values = color_pal) +
  theme_minimal() + 
  theme(legend.position = "bottom", 
        text = element_text(size = 18),
