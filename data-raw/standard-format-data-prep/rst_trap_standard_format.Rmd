...
- flow_start_meter, flow_end_meter, flow_set_time, velocity, turbidity,
weather_code, lunar_phase, diel, river_left_depth, river_center_depth, river_right_depth included in environmental table
...
knights_trap_clean_2006 <- knights_effort %>%
  select(-c(counterAtStart, counterAtEnd, visitTime2)) %>%
...
tisdale_trap_clean <- tisdale_trap_camp %>%
  select(-c(sampleGear, visitTime2)) %>%
  mutate(stream = "sacramento river",
         site = "tisdale",
         subsite = as.character(subSiteName)) %>%
  rename(trap_visit_id = trapVisitID,
...
unique(combined_trap$trap_status)
unique(combined_trap$gear_condition)
...
rst_trap_data_dictionary <- tibble(variable_name = colnames(combined_trap),
                                    description = c("Key variable that links trap operations to catch data and environmental data. Currently not all locations use this key.", #1
                                                    "Date when trap was started or restarted.", #2
                                                    "Time when trap was started or restarted.", #3
                                                    "Date when trap was checked and catch counted. The start date is typically the end date from the previous day.", #4
                                                    "Time when trap was checked and catch counted. The start time is typically the end time from the previous day.", #5
                                                    "Number of cone revolutions during sampling period.", #6
                                                    "Was trap fishing at half cone (T/F)", #7
                                                    "Was trap fishing in the thalweg (T/F)",#8
                                                    "The depth of the bottom of the cone (units = cm)",#9
                                                    "Qualitative comments about sampling event.",#10
                                                    "A code for the condition of the trap", #11
                                                    "Did the trap fish for the entire sample day (T/F)",#12
                                                    "Location of monitoring program (e.g. stream or river)",#13
                                                    "Site within the location",#14
                                                    "Subsite or specific trap location within each site and location. Names are specific to location.",#15
                                                    "Volume of debris measured in trap (units = gallons).",#16
                                                    "Cone rate measurement - revolutions per mintue at the start of trap visit", #17
                                                    "Status of trap during visit",#18
                                                    "Cone rate  measurement - revolutions per minute at the end of the trap visit",#19
                                                    "Revolutions on counter at start of sampling visit",#20
                                                    "Revolutions on counter at end of sampling visit",#21
                                                    "Description of whether fish were processed",#22
                                                    "Method of sampling. (Categories: fish screen diversion trap, rotary screw trap)",#23
                                                    "Categorical level of debris in trap (visual assessment).",#24
                                                    "Number of traps operating"),#25
                                    
                                   encoding = c(NA, NA, NA, NA, NA, 
                                                NA,
                                                "F = full cone, T = half cone",
                                                "F = not in thalweg, T = in thalweg",
                                                NA, NA, 
                                                "not recorded, trap functioning normally, trap stopped functioning, trap not in service, trap functioning but not normally",
                                                "T = only partial sample day, F = full sample day",
                                                NA, NA, NA, NA, NA,
                                                "not recorded, drive by, end trapping, start trapping, continue trapping, unplanned restart, service trap",
                                                NA, NA, NA,
                                                "not recorded, processed fish, no fish caught, no catch data fish released, no catch data fish left in live box",
                                               "fish screen diversion trap, rotary screw trap",
                                                "light, medium, heavy, very heavy",
                                                NA
                                                ),
                                   collected = c("Battle, Clear, Knights, Tisdale", #1
                                                 "Battle, Clear", #2
                                                 "Battle, Clear", #3
                                                 "Battle, Butte, Clear, Deer, Feather, Mill, Yuba, Knights, Tisdale", #4
                                                 "Battle, Butte, Clear, Yuba, Knights",
                                                 "Battle, Butte, Clear, Yuba, Knights",
                                                 "Battle, Clear, Feather, Knights, Tisdale",
                                                 "Battle, Clear, Feather, Knights, Tisdale",
                                                 "Battle, Clear",
                                                 "Battle, Butte, Clear, Deer, Feather, Mill, Yuba, Knights, Tisdale",
                                                 "Battle, Clear, Deer, Feather, Mill, Knights, Tisdale",
                                                 "Battle, Clear",
                                                 "All",
                                                 "All",
                                                 "All",
                                                 "Battle, Clear",
                                                 "Battle, Butte, Clear, Deer, Feather, Mill, Yuba, Knights, Tisdale",
                                                 "Butte, Feather, Yuba, Knights, Tisdale",
                                                 "Butte, Feather, Yuba, Knights, Tisdale",
                                                 "Feather, Knights, Tisdale",
                                                 "Feather, Knights, Tisdale",
                                                  "Feather, Knights, Tisdale",
                                                 "Yuba",
                                                 "Yuba, Knights, Tisdale",
                                                 "Knights")
                                    )
```
