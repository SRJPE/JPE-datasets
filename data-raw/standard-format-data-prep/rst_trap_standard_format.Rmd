...
butte_trap_clean_pre_2015 <- butte_rst %>% 
  ...
  mutate(stream = "butte creek",
         subsite = ifelse(gear_id == "diversion fyke trap 1", "okie fyke trap", tolower(site)), 
...
butte_trap_clean_camp <- butte_trap_camp |> 
  ...
  mutate(stream = "butte creek",
         site = "okie dam", # Only fish at okie dam now - also refered to as parot-phelan
         subsite = ifelse(subSiteName == "PP RST", "okie dam 1", "okie fyke trap"),
         ...
         trap_stop_time = case_when(visitType %in% c("Continue trapping", "Unplanned restart", "End trapping") ~ as_hms(visitTime),
                                      T ~ NA_real_),
         trap_start_date = case_when(visitType %in% c("Continue trapping", "Unplanned restart", "End trapping") | (fishProcessed == "Processed fish" & visitType != "Start trap & begin trapping") ~ as.Date(lag(visitTime2)),
                                     T ~ NA_real_),
         trap_start_time = case_when(visitType %in% c("Continue trapping", "Unplanned restart", "End trapping") | (fishProcessed == "Processed fish" & visitType != "Start trap & begin trapping") ~ as_hms(lag(visitTime2)),
                                     T ~ NA_real_)) %>% 
...
butte_trap_clean <- bind_rows(butte_trap_clean_pre_2015, butte_trap_clean_camp)
