...
battle_trap_clean <- battle_environmental %>%
  ...
  rename(trap_visit_id = sample_id,
         trap_sample_date = sample_date,
         trap_sample_time = sample_time,
...
butte_trap_clean <- butte_rst %>%
  ...
  rename(site = station,
         ...
         trap_sample_date = date,
         trap_sample_time = time) %>%
...
feather_trap_clean <- feather_effort %>%
  ...
  rename(site = site_name,
         ...
         trap_sample_time = visit_time,
         trap_sample_date = date,
         trap_status = visit_type,
         gear_condition_code = trap_functioning) %>%
  mutate(trap_sample_time = as_hms(trap_sample_time),
...
knights_trap_clean <- knights_effort %>%
  ...
  select(-cone_id) %>%
  rename(trap_sample_date = stop_date,
         trap_sample_time = stop_time,
...
combined_trap <- combined_trap_raw %>%
  ...
  mutate(debris_level = tolower(debris_level),
         ...
         gear_condition_code = case_when(gear_condition_code %in% c("total block", "total blockage", "Trap not in service") ~ "trap not in service",
...
check <- combined_trap %>%
  filter(!is.na(trap_start_date)) %>%
  group_by(location, site, subsite) %>%
  arrange(trap_start_date) %>%
  mutate(date_ck = lag(trap_sample_date)) %>%
  filter(date_ck != trap_start_date) %>%
  select(trap_start_date, trap_sample_date, date_ck, location, site, subsite)
...
```{r}
combined_trap %>% 
  ggplot(aes(x = trap_sample_date, y = location, color = location)) +
  geom_point()

combined_trap %>% 
  ggplot(aes(x = trap_start_date, y = location, color = location)) +
  geom_point()
```
...
```{r}
combined_trap %>% 
  ggplot(aes(x = trap_sample_time, y = location, color = location)) +
  geom_point()

combined_trap %>% 
  ggplot(aes(x = trap_start_time, y = location, color = location)) +
  geom_point()
```
...
```{r}
combined_trap %>%
  ggplot(aes(x = trap_sample_date, y = sample_period_revolutions, color = location)) +
  geom_point()
...
```{r}
combined_trap %>%
  filter(!is.na(cone_setting)) %>%
  group_by(location, cone_setting) %>%
  tally() %>%
  kable()
```
...
```{r}
combined_trap %>%
  ggplot(aes(x = trap_sample_date, y = river_left_depth, color = location)) +
  geom_point()
combined_trap %>%
  ggplot(aes(x = trap_sample_date, y = river_right_depth, color = location)) +
  geom_point()
combined_trap %>%
  ggplot(aes(x = trap_sample_date, y = river_center_depth, color = location)) +
  geom_point()
combined_trap %>%
  ggplot(aes(x = trap_sample_date, y = depth_adjust, color = location)) +
  geom_point()
...
```{r}
combined_trap %>%
  filter(!is.na(thalweg)) %>%
  group_by(location, thalweg) %>%
  tally() %>%
  kable()
```
...
```{r}
combined_trap %>%
  ggplot(aes(x = trap_sample_date, y = avg_time_per_rev, color = location)) +
  geom_point()

combined_trap %>%
  ggplot(aes(x = location, y = avg_time_per_rev)) +
  geom_boxplot()
```
...
combined_trap %>%
  ...
  group_by(location, fish_properly) %>%
  tally() %>%
  kable()
```
...
total <-
  combined_trap %>%
  group_by(year(trap_sample_date), location) %>%
  tally() %>%
  rename(total = n)
combined_trap %>%
  group_by(year(trap_sample_date), location, gear_condition_code) %>%
  tally() %>%
  left_join(total) %>%
  mutate(percent = n/total) %>%
  ggplot(aes(x = `year(trap_sample_date)`, y = percent, fill = gear_condition_code)) +
...
combined_trap %>%
  ...
  group_by(location, trap_fishing) %>%
  tally() %>%
  kable()
```
...
combined_trap %>%
  ...
  group_by(location, partial_sample) %>%
  tally() %>%
  kable()
```
...
```{r}
combined_trap %>%
  ggplot(aes(x = trap_sample_date, y = debris_volume, color = location)) +
  geom_point()

combined_trap %>%
  ggplot(aes(x = location, y = debris_volume)) +
  geom_boxplot()
```
...
```{r}
combined_trap %>%
  group_by(year(trap_sample_date), location, trap_status) %>%
  tally() %>%
  left_join(total) %>%
  mutate(percent = n/total) %>%
  ggplot(aes(x = `year(trap_sample_date)`, y = percent, fill = trap_status)) +
  geom_bar(stat = "identity", position = "stack") +
  facet_wrap(~location)
...
```{r}
combined_trap %>%
  ggplot(aes(x = trap_sample_date, y = rpms_start, color = location)) +
  geom_point()
...
filter(combined_trap, rpms_start < 100) %>%
  ...
  geom_point()
combined_trap %>%
  ggplot(aes(x = trap_sample_date, y = rpms_end, color = location)) +
  geom_point()

combined_trap %>%
  ggplot(aes(x = location, y = rpms_start)) +
  geom_boxplot()
combined_trap %>%
  ggplot(aes(x = location, y = rpms_end)) +
  geom_boxplot()
```
...
```{r}
combined_trap %>%
  group_by(year(trap_sample_date), location, fish_processed) %>%
  tally() %>%
  left_join(total) %>%
  mutate(percent = n/total) %>%
  ggplot(aes(x = `year(trap_sample_date)`, y = percent, fill = fish_processed)) +
  geom_bar(stat = "identity", position = "stack") +
  facet_wrap(~location)
...
```{r}
combined_trap %>%
  group_by(year(trap_sample_date), location, method) %>%
  tally() %>%
  left_join(total) %>%
  mutate(percent = n/total) %>%
  ggplot(aes(x = `year(trap_sample_date)`, y = percent, fill = method)) +
  geom_bar(stat = "identity", position = "stack") +
  facet_wrap(~location)
```
...
```{r}
combined_trap %>%
  group_by(year(trap_sample_date), location, debris_level) %>%
  tally() %>%
  left_join(total) %>%
  mutate(percent = n/total) %>%
  ggplot(aes(x = `year(trap_sample_date)`, y = percent, fill = debris_level)) +
  geom_bar(stat = "identity", position = "stack") +
  facet_wrap(~location)
```
...
combined_trap %>%
  ...
  group_by(location, number_traps) %>%
  tally() %>%
  kable()
```
...
```{r}
combined_trap %>%
  ggplot(aes(x = trap_sample_date, y = cone_rpm, color = location)) +
  geom_point()
```
