...
check <- combined_trap %>%
  ...
  select(trap_start_date, trap_sample_date, date_ck, stream, site, subsite)
...
```{r}
combined_trap %>% 
  ggplot(aes(x = trap_sample_date, y = stream, color = stream)) +
  geom_point()
...
```{r}
combined_trap %>% 
  ggplot(aes(x = trap_sample_time, y = stream, color = stream)) +
  geom_point()
...
```{r}
combined_trap %>%
  ggplot(aes(x = trap_sample_date, y = sample_period_revolutions, color = stream)) +
  geom_point()
...
```{r}
combined_trap %>%
  ggplot(aes(x = trap_sample_date, y = river_left_depth, color = stream)) +
  geom_point()
combined_trap %>%
  ggplot(aes(x = trap_sample_date, y = river_right_depth, color = stream)) +
  geom_point()
combined_trap %>%
  ggplot(aes(x = trap_sample_date, y = river_center_depth, color = stream)) +
  geom_point()
combined_trap %>%
  ggplot(aes(x = trap_sample_date, y = depth_adjust, color = stream)) +
  geom_point()
...
```{r}
combined_trap %>%
  ggplot(aes(x = trap_sample_date, y = avg_time_per_rev, color = stream)) +
  geom_point()
...
total <-
  combined_trap %>%
  group_by(year(trap_sample_date), stream) %>%
  tally() %>%
  rename(total = n)
combined_trap %>%
  group_by(year(trap_sample_date), stream, gear_condition) %>%
  tally() %>%
  left_join(total) %>%
  mutate(percent = n/total) %>%
  ggplot(aes(x = `year(trap_sample_date)`, y = percent, fill = gear_condition)) +
...
```{r}
combined_trap %>%
  ggplot(aes(x = trap_sample_date, y = debris_volume, color = stream)) +
  geom_point()
...
```{r}
combined_trap %>%
  group_by(year(trap_sample_date), stream, trap_status) %>%
  tally() %>%
  left_join(total) %>%
  mutate(percent = n/total) %>%
...
```{r}
combined_trap %>%
  ggplot(aes(x = trap_sample_date, y = rpms_start, color = stream)) +
  geom_point()
...
filter(combined_trap, rpms_start < 100) %>%
  ...
  geom_point()
combined_trap %>%
  ggplot(aes(x = trap_sample_date, y = rpms_end, color = stream)) +
  geom_point()
...
```{r}
combined_trap %>%
  group_by(year(trap_sample_date), stream, fish_processed) %>%
  tally() %>%
  left_join(total) %>%
  mutate(percent = n/total) %>%
...
```{r}
combined_trap %>%
  group_by(year(trap_sample_date), stream, method) %>%
  tally() %>%
  left_join(total) %>%
  mutate(percent = n/total) %>%
...
```{r}
combined_trap %>%
  group_by(year(trap_sample_date), stream, debris_level) %>%
  tally() %>%
  left_join(total) %>%
  mutate(percent = n/total) %>%
...
```{r}
combined_trap %>%
  ggplot(aes(x = trap_sample_date, y = cone_rpm, color = stream)) +
  geom_point()
```
