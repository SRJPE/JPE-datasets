...
# Combine releases and recaptures 
...
summarized_mark_recapture <- standard_releases %>% 
  ...
  summarize(caught_week_1 = sum(caught_week_1, na.rm = T),
            ...
            median_fork_length_recaptured = median(median_fork_length_recaptured, na.rm = T)) %>% glimpse


```
## Reformat to Josh's format
...
weekly_releases_and_recaptures <- summarized_mark_recapture %>% 
  ...
  mutate(m2 = ifelse(stream %in% c("battle creek", "clear creek"), NA, m2), # Replace 0 that were created in summarize statement with NA
         m3 = ifelse(stream %in% c("battle creek", "clear creek"), NA, m3)) %>% glimpse
...
write_csv(releases_and_recaptures, "../../data/jpe-model-data/jpe_releases_and_recaptures.csv")
...
gcs_upload(releases_and_recaptures,
           object_function = f,
           type = "csv",
           name = "jpe-model-data/releases_and_recaptures.csv")
```
