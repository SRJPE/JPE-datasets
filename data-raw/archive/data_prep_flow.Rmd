...
josh_format_flow_daily%>% 
  # filter(stream == "sacramento river") %>%
  ggplot() +
  geom_line(aes(x = date, y = mean_daily_flow, color = site), linetype = "dashed", alpha = .25) + 
  theme_minimal() +
...
josh_format_flow_weekly <- josh_format_flow_daily %>%
  group_by(site, stream, jwk, yr) %>%
  summarize(mean_weekly_flow = mean(mean_daily_flow, na.rm = T)) %>%
  left_join(weekly_na)


unique(josh_format_flow_weekly$site)
View(josh_format_flow_weekly)
```
...
```{r}
weekly_efficiency_with_flow <- weekly_mark_recapture %>% 
  rename(jwk = Jwk) %>%
  left_join(josh_format_flow_weekly) %>%
  filter(!is.na(site), r1 > 0) %>% # 2 NA values - filtering out 
  filter(r1 > m1) %>% # remove one value on battle creek in 2018 where raw data shows more recaptures than released (data entry error)
  glimpse

View(weekly_efficiency_with_flow)

write_csv(weekly_efficiency_with_flow, "../../data/jpe-model-data/jpe_weekly_efficiency_with_flow.csv")
```
```{r}
unique(weekly_efficiency_with_flow$stream)
unique(weekly_efficiency_with_flow$site)

weekly_efficiency_with_flow %>% filter(is.na(site))
unique(weekly_efficiency_with_flow$yr)
unique(weekly_efficiency_with_flow$jwk)

weekly_efficiency_with_flow %>% filter(r1 == 0)

summary(weekly_efficiency_with_flow$r1) #investigage 0 values for r1
summary(weekly_efficiency_with_flow$m1)
weekly_efficiency_with_flow %>% filter(m1 > 1000)

summary(weekly_efficiency_with_flow$m2)
summary(weekly_efficiency_with_flow$m3)

summary(weekly_efficiency_with_flow$average_weekly_flow)
