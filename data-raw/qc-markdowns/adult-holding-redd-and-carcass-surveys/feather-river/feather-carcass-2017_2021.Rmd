...
chop_join <- full_join(ChopHeader_raw |> 
  ...
  mutate(way_pt = as.character(way_pt),
         ...
         creation_time = as.POSIXct(gsub("[()]", "", creation_time), format = "%m/%d/%y %H:%M:%S")) |> glimpse()
...
chop_recovery_cleaner <- chop_recovery_join |> 
  mutate(way_pt = as.character(way_pt),
         r_mile = as.numeric(r_mile),
         sex = case_when(sex_id == 251 ~ "unknown",
                         sex_id == 253 ~ "not recorded", 
                         sex_id == 1 ~ "female",
                         sex_id == 2 ~ "male")) |> 
  ...
  mutate(chop_clip_status = ifelse(disposition == "chopped", "yes", "no"),
         tag_col = as.character(color_tag_recovered_id)) |> 
    select(-disposition_id, disposition, -sex_id, color_tag_recovered_id) |> 
           glimpse()
# TODO chop_recovery contains a lot of the data that is in the chop_tags from other years:
# TODO: can we connect chop_recovery_cleaner to chop_cleaner to get chop_clip_status, sex, fl, tag col (all in chop table from other years)
# TODO we could join on survey_meta_id, date, survey_id, way_pt r_mile

# TODO no lifestage data
# TODO can we get tag_clip_status?
```
...
feather_carcass_chop_header_2017<- chop_header_cleaner |> 
  filter(year(date) == 2017)
feather_carcass_chop_recovery_2017<- chop_recovery_cleaner |> 
  filter(year(date) == 2017)
...
feather_carcass_chop_header_2018 <- chop_header_cleaner |> 
  filter(year(date) == 2018)
feather_carcass_chop_recovery_2018 <- chop_recovery_cleaner |> 
  filter(year(date) == 2018)
#2019
...
feather_carcass_chop_header_2019<- chop_header_cleaner |> 
  filter(year(date) == 2019)
feather_carcass_chop_recovery_2019 <- chop_recovery_cleaner |> 
  filter(year(date) == 2019)
#2020
...
feather_carcass_chop_header_2020<- chop_header_cleaner |> 
  filter(year(date) == 2020)
feather_carcass_chop_recovery_2020 <- chop_recovery_cleaner |> 
  filter(year(date) == 2020)
...
gcs_upload(feather_carcass_chop_recovery_2017,
           object_function = f,
           type = "csv",
           name = "adult-holding-redd-and-carcass-surveys/feather-river/data/feather_carcass_chop_recovery_2017.csv")
...
gcs_upload(feather_carcass_chop_recovery_2018,
           object_function = f,
           type = "csv",
           name = "adult-holding-redd-and-carcass-surveys/feather-river/data/feather_carcass_chop_recovery_2018.csv")
...
gcs_upload(feather_carcass_chop_recovery_2019,
           object_function = f,
           type = "csv",
           name = "adult-holding-redd-and-carcass-surveys/feather-river/data/feather_carcass_chop_recovery_2019.csv")
...
gcs_upload(feather_carcass_chop_recovery_2020,
           object_function = f,
           type = "csv",
           name = "adult-holding-redd-and-carcass-surveys/feather-river/data/feather_carcass_chop_recovery_2020.csv")
```
