...
cwt <- left_join(cwt_raw |> clean_names(), 
  ...
  mutate(source_table = "CWT",
         ...
         creation_time = as.POSIXct(gsub("[()]", "", creation_time), 
                                    format = "%m/%d/%y %H:%M:%S")) |>
  select(-c(survey_meta_id, individual_id, survey_id, fl_cm, comments, data_recorder, creation_time, editor, edit_time, cwt_cd, boat, time_in, time_out, field_recorder, crew, date, subsample)) |> 
