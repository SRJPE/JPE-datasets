...
#2. chops and tags
chops_and_tags_join <- chops_and_tags_raw |> 
  left_join(ChopHeader_raw |> 
              select(SurveyMetaID, SurveyID, Date, SectionID), by = c("SurveyMetaID", "SurveyID")) |> 
  clean_names() |> 
  left_join(species_LU |> 
              mutate(species_id = as.numeric(TaxonID),
                     species = CommonName) |> 
...
#3. chop recovery
chop_recovery_join <- ChopRecov_raw |> 
  left_join(ChopHeader_raw |> 
               select(SurveyMetaID, SurveyID, Date, SectionID),
             by = c("SurveyMetaID", "SurveyID")) |> 
  clean_names() |> 
  left_join(disposition_LU |> select(DispositionID, disposition = Disposition), 
            by = c("disposition_id" = "DispositionID")) |>
  left_join(color_LU |> 
...
### Look at total carcasses - bind together `chops_and_tags_join`, `chop_join`, and `chop_recovery_join`

```{r}
all_carcasses <- bind_rows(chop_join |> mutate(source = "chop") |> 
                             select(date, count, source, disposition = chop_type_cd, species, section_id, run),
                           chop_recovery_join |> mutate(source = "recovery") |> 
                                                          select(date, count, source, disposition, sex, fl_mm),
                           chops_and_tags_join |> mutate(source = "chops_and_tags") |>  select(date, count, source, fl_mm, species, run, disposition, spawned, ad_fin_clip_status, sex)) |>
  mutate_if(is.character, str_to_lower) |> 
  mutate(disposition = case_when(disposition == "chop" ~ "chopped",
                                 disposition == "not chopped" ~ "recaptured",
                                 TRUE ~ disposition)) |> 
  glimpse()
...
all_carcasses |> 
  mutate(year = year(date),
         month = month(date)) |> 
  ggplot(aes(x = month, y = count, fill = disposition)) +
  geom_col() +
  facet_wrap(~year) + 
  theme_minimal()
