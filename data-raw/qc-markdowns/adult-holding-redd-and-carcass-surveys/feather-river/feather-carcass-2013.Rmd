...
cwt <- left_join(cwt_raw |> clean_names(), 
  ...
  mutate(source_table = "CWT",
         ...
         disposition = case_when(recapture_or_chop == "C" ~ "chopped",
                                 ...
                                 recapture_or_chop == "R/C" ~ "recaptured, chopped",
...
### All Carcasses Variables: `header_id`, `week`, `sect`, `min`, `count`, `cwt_id`, `river_section`, `tag_id`, `fl`, `head_tag_number`, `scales`, `otoliths`, `section_group_1_10`, `section_group_11_15`
...
Carcasses in the `all_carcasses` dataset are marked as either `tagged`, `recaptured`, `recaptured, chopped`, or  `chopped`. Carcasses from the `CWT` table are marked as `chopped`, `tagged`, `recaptured`, `recaptured, chopped`, or `NA` and have individual-level sampling information (i.e sex, fork length, adipose fin clip status). `chopped` carcasses from `ChopJoin` do not have individual-level sampling data. 
...
```{r}
all_carcasses |> 
  filter(fl < 600) |> # for scaling plot
...
all_carcasses_cleaner <- all_carcasses |> 
  ...
  mutate(spawning_condition = case_when(spawning_condition == "u" ~ "unspawned",
         ...
         tag_color = case_when(tag_color == "blue tag" ~ "blue",
                               str_detect(tag_color, "decay") ~ "decayed",
                               str_detect(tag_color, "all unknown") ~ "silver",
                               str_detect(tag_color, "datasheets") ~ NA_character_,
                                TRUE ~ tag_color)) |> 
...
chop_header_cleaner <- chop_header |> 
  ...
  mutate(crew = str_replace_all(crew, " ", ""),
         ...
         weather = case_when(weather == "cld" ~ "cloudy",
                             ...
                             weather %in% c("cld/rain", "cld / rain") ~ "rainy",
                             weather == "rain" ~ "rainy",
                             weather == "clr" ~ "sun",
...
* Same table/layout structure as 2011-2012. There is only a bulk chops table, `ChopJoin`, and a CWT table with individual carcass sampling data.
...
```{r}
all_carcasses_2013 <- all_carcasses_cleaner |> glimpse()
```

```{r, include=FALSE}
f <- function(input, output) write_csv(input, file = output)

gcs_upload(all_carcasses_2013,
           object_function = f,
           type = "csv",
           name = "adult-holding-redd-and-carcass-surveys/feather-river/data/feather_carcass_all_carcasses_2013.csv")
```
