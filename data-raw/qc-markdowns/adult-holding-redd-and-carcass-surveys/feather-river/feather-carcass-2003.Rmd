...
# TODO no ChanID in ChopHeader_raw
...
filter(chop_channel_join) |> 
  group_by(date) |> 
  summarize(female_chop = sum(female_chop)) |> 
  ggplot(aes(x = date, y = female_chop)) +
  geom_col(size = 1.4, alpha = .5, fill = "blue") + 
  labs(x = "Date", 
       y = "Female Chop") +
  theme_minimal() + 
...
filter(chop_channel_join) |> 
  ...
  geom_col(size = 1.4, alpha = .5, fill = "red") + 
  labs(x = "Date", 
       y = "Male Chop") +
  theme_minimal() + 
  theme(text = element_text(size = 15)) 
...
filter(chop_channel_join) |> 
  group_by(date) |> 
  summarise(grilse = sum(grilse)) |> 
  ggplot(aes(x = date, y = grilse)) +
  geom_col(size = 1.4, alpha = .5, fill = "black") + 
  labs(x = "Date", 
       y = "Grilse") +
  theme_minimal() + 
  theme(text = element_text(size = 15)) 
...
chop_channel_join |> 
  group_by(date) |> 
  summarise(tag_count = sum(tag_count)) |> 
...
# TODO: I think from reading the sampling protocol, the tags recovered on a specific date are likely from the previous two weeks. So this plot is essentially showing us the proportion of chops recovered on a specific date that were tagged previously, i.e. giving us an idea of how many in the population are tagged. Check in on this with Ashley to confirm understanding.
...
* `r round(sum(is.na(cwt$header_id))/nrow(cwt), 3) * 100` % of values in the `header_id` column are NA.
...
```{r}
cwt |> 
  mutate(sex = case_when(sex == "nd" ~ "Unknown",
                         TRUE ~ sex))|> 
