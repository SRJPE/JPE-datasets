---
title: "feather-river-redd-survey-qc-checklist-2018"
author: "Inigo Peng"
date: "10/6/2021"
...
* `r round(sum(is.na(cleaner_data_2018$'date'))/nrow(cleaner_data_2018), 3)*100` % of values in the `date` column are NA.
...
* `r round(sum(is.na(cleaner_data_2018$location))/nrow(cleaner_data_2018), 3)*100` % of values in the `location` column are NA.
...
**Numeric Daily Summary of salmon_count Over 2018**
...
**Numeric summary of salmon_count by location in 2018**
```{r}
cleaner_data_2018 %>%
  group_by(location) %>% 
  summarise(count = sum(salmon_count, na.rm = T)) %>% 
...
* `r round(sum(is.na(cleaner_data_2018$salmon_count))/nrow(cleaner_data_2018), 5)*100` % of values in the `salmon_count` column are NA. 
...
**Numeric Daily Summary of redd_count Over 2018**
```{r}
cleaner_data_2018 %>%
  group_by(date) %>%
...
* `r round(sum(is.na(cleaner_data_2018$redd_count))/nrow(cleaner_data_2018), 5)*100` % of values in the `redd_count` column are NA.
...
**Numeric Summary of redd_width_m Over 2018**
```{r}
...
* `r round(sum(is.na(cleaner_data_2018$'redd_width_m'))/nrow(cleaner_data_2018), 3)*100` % of values in the `redd_width_m` column are NA.
...
**Numeric Summary of redd_length_m Over 2018**
```{r}
summary(cleaner_data_2018$redd_length_m)
```
**NA and Unknown Values**

* `r round(sum(is.na(cleaner_data_2018$'redd_length_m'))/nrow(cleaner_data_2018), 3)*100` % of values in the `redd_length_m` column are NA.
...
* `r round(sum(is.na(cleaner_data_2018$'longitude'))/nrow(cleaner_data_2018), 3)*100` % of values in the `longitude` column are NA.  

* `r round(sum(is.na(cleaner_data_2018$'latitude'))/nrow(cleaner_data_2018), 3)*100` % of values in the `latitude` column are NA.
...
**Numeric Summary of percent_fine_substrate Over 2018**
```{r}
...
* `r round(sum(is.na(cleaner_data_2018$'percent_fine_substrate'))/nrow(cleaner_data_2018), 3)*100` % of values in the `percent_fine_substrate` column are NA.
...
**Numeric Summary of percent_small_substrate Over 2018**
```{r}
...
* `r round(sum(is.na(cleaner_data_2018$'percent_small_substrate'))/nrow(cleaner_data_2018), 3)*100` % of values in the `percent_small_substrate` column are NA.
...
**Numeric Summary of percent_medium_substrate Over 2018**
```{r}
...
* `r round(sum(is.na(cleaner_data_2018$'percent_medium_substrate'))/nrow(cleaner_data_2018), 3)*100` % of values in the `percent_medium_substrate` column are NA.
...
**Numeric Summary of percent_large_substrate Over 2018**
```{r}
...
* `r round(sum(is.na(cleaner_data_2018$'percent_large_substrate'))/nrow(cleaner_data_2018), 3)*100` % of values in the `percent_large_substrate` column are NA.
...
**Numeric Summary of percent_boulder Over 2018**
```{r}
summary(cleaner_data_2018$percent_large_substrate)
```
...
* `r round(sum(is.na(cleaner_data_2018$'percent_boulder'))/nrow(cleaner_data_2018), 3)*100` % of values in the `percent_boulder` column are NA.
...
**Numeric Summary of depth_m Over 2018**
```{r}
...
* `r round(sum(is.na(cleaner_data_2018$'depth_m'))/nrow(cleaner_data_2018), 3)*100` % of values in the `depth_m` column are NA.
...
**Numeric Summary of pot_depth_m Over 2018**
```{r}
...
* `r round(sum(is.na(cleaner_data_2018$'pot_depth_m'))/nrow(cleaner_data_2018), 3)*100` % of values in the `pot_depth_m` column are NA.
...
**Numeric Summary of velocity_m_per_s Over 2018**
```{r}
...
* `r round(sum(is.na(cleaner_data_2018$'velocity_m_per_s'))/nrow(cleaner_data_2018), 3)*100` % of values in the `velocity_m_per_s` column are NA.
