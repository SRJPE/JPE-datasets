...
cwt <- left_join(cwt_raw |> clean_names(), 
  ...
  mutate(disposition = case_when(disposition == "tag_id" ~ "tagged",
         ...
         count = 1) |> 
  filter(!is.na(tag_id)) |> 
  select(-c(morale, lfc, hfc, radio_tag_id))  |> # lfc, hfc, radio tag empty; morale not helpful
  glimpse()
...
all_carcasses_cleaner <- all_carcasses |> 
  ...
  select(-c(crew)) |>  # time is removed because no stop time provided so information is not useful. Not sure section_group variables are useful
  glimpse()
```
