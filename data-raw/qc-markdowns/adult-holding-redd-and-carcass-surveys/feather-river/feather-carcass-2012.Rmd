...
chop_join <- full_join(ChopHeader_raw |> 
                                 select(Header.ID, Date, Comments),
                               Chops_raw,
                       by = "Header.ID") |> 
  clean_names() |> 
  rename(sect ="section",
...
### CWT
The `cwt` table contains biological sampling data for carcasses. It does not contain information about whether the carcass was chopped or tagged. Each `tag_id` is only represented once in the table.
...
### Chop Join Variable: `header_id`, `min`, `sect`
```{r}
chop_join |> 
...
round(sum(is.na(chop_join$sect))/nrow(chop_join), 3) * 100
```
...
* `r round(sum(is.na(chop_join$sect))/nrow(chop_join), 3) * 100` % of values in the `sect` column are NA.
