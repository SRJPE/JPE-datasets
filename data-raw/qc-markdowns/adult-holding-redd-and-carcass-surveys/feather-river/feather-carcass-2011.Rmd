...
### CWT
The `cwt` table contains sampling information on individual carcasses. This table also contains sex, spawning condition, and adipose fin clip status. It is not joined with the other tables. Added a `count` column assuming each row represents one individual. This table contains `disposition` information in the `recapture_or_chop` variable (in previous years this was the `carc_stat` variable). If a tagged carcass was tagged, recaptured, re-released, and chopped, it will be represented multiple times in this table (i.e. there can be multiple rows for one `tag_id`).  Added `source_table` column for tracking.
...
cwt <- left_join(cwt_raw |> clean_names(), 
  ...
  mutate(source_table = "CWT",
         ...
         sex = case_when(sex == "F" ~ "female",
                         sex == "M" ~ "male",
                         TRUE ~ sex)) |>
  select(-c(recapture_or_chop, cwt_id, header_id, crew, tag, morale, comments, section_group_1_10, section_group_11_15, section_group_16_21, section_group_22_38)) |> 
  glimpse()
```
...
all_carcasses_cleaner <- all_carcasses |> 
  ...
  rename(head_tag = head_tag_number,
         tag_col = tag_recap_chop_color) |> 
  select(-c(time)) |>  # time is removed because no stop time provided so information is not useful. Not sure section_group variables are useful
  glimpse()
```
