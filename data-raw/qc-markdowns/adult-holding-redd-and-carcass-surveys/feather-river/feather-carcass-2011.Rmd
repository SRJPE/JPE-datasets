...
```{r}
# "k" = "unknown" (using Feather River Carcass Survey Plans 2021 document)
cwt_cleaner <- cwt |> 
  mutate_if(is.character, str_to_lower) |> 
  mutate(crew = str_replace_all(crew, " ", ","),
         crew = str_replace_all(crew, ",,", ","),
         ...
         adipose_fin_clipped = case_when(adipose_fin_clipped == "k" ~ "unknown",
                                         adipose_fin_clipped == "y" ~ "yes",
                                         adipose_fin_clipped == "n" ~ "no",
                                         TRUE ~ adipose_fin_clipped),
         spawning_condition = case_when(spawning_condition == "k" ~ "unknown",
                                        spawning_condition == "s" ~ "spawned",
                                        spawning_condition == "u" ~ "unspawned",
...
chop_header_dictionary <- tibble(variables = colnames(chop_header_cleaner),
                          description = c("Date of survey",
                                          "Week Number",
                                          "Weather",
                                          "Initials of Crew Members Collecting Data",
                                          "Comments",
                                          "Header ID"),
                          percent_na = round(percent_na$value*100))
# 
