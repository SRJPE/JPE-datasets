...
```{r, include=FALSE}
gcs_get_object(object_name = "adult-holding-redd-and-carcass-surveys/feather-river/data-raw/carcass/Carcass_2011.mdb",
               bucket = gcs_get_global_bucket(),
               saveToDisk = "data-raw/qc-markdowns/adult-holding-redd-and-carcass-surveys/feather-river/feather_carcass_2011.mdb",
               overwrite = TRUE)

filepath <- "data-raw/qc-markdowns/adult-holding-redd-and-carcass-surveys/feather-river/feather_carcass_2011.mdb"
...
if(operating_system == "pc") {
  ...
  # Switchboard Items and Sample Weeks are empty. Paste Errors and Otoliths table not relevant to modeling.
...
write_csv(Chops_raw, here::here("data-raw", "qc-markdowns", "adult-holding-redd-and-carcass-surveys", "feather-river", "Chops_2011.csv"))
write_csv(ChopHeader_raw, here::here("data-raw", "qc-markdowns", "adult-holding-redd-and-carcass-surveys", "feather-river", "ChopHeader_2011.csv"))
write_csv(cwt_raw, here::here("data-raw", "qc-markdowns", "adult-holding-redd-and-carcass-surveys", "feather-river", "CWT_2011.csv"))
write_csv(cwt_header_raw, here::here("data-raw", "qc-markdowns", "adult-holding-redd-and-carcass-surveys", "feather-river", "CWTHeader_2011.csv"))
```
...
Chops_raw <- read_csv(here::here("data-raw", "qc-markdowns",
                                 ...
                                 "feather-river", "Chops_2011.csv")) |>
  rename(Header.ID = Chop.Header.ID) |> 
  select(-`Chop.ID`) |> # this column is all NAs
  glimpse()
```
...
ChopHeader_raw <- read_csv(here::here("data-raw", "qc-markdowns",
                                 "adult-holding-redd-and-carcass-surveys",
                                 "feather-river", "ChopHeader_2011.csv")) |> 
  rename(Header.ID = `Chop.Header.ID`) |>
  glimpse()
```
...
colnames(cwt_raw) <- c("CWT.ID", "Header.ID", "River.Section", "Tag.Recap.Chop.Color", "Tag.ID", "Recapture.or.Chop", "Sex",
                       "Spawning.Condition", "Adipose.Fin.Clipped", "Samples.Collected",
                       "Fork.Length", "Head.Tag.Number", "Scales", "Otoliths", "Hallprint", 
                       "Comments", "Tag")
...
cwt_header_raw <- read_csv(here::here("data-raw", "qc-markdowns",
                                 "adult-holding-redd-and-carcass-surveys",
                                 "feather-river", "CWTHeader_2011.csv")) |> 
  rename(Header.ID = `CWT.Header.ID` ) |> 
  glimpse()
```
...
### CWT
The `cwt` table contains sampling information on individual carcasses. This table also contains sex, spawning condition, and adipose fin clip status. It is not joined with the other tables. Added a `count` column assuming each row represents one individual. This table contains `disposition` information in the `recapture_or_chop` variable (in previous years this was the `carc_stat` variable. f a tagged carcass was tagged, recaptured, re-released, and chopped, it will be represented multiple times in this table (i.e. there can be multiple rows for one `tag_id`).  Added `source_table` column for tracking. Each `tag_id` is represented only once in this table.
...
### All Carcasses Variables: `header_id`, `sect`, `min`, `count`, `cwt_id`, `river_section`, `tag_id`, `fl`, `head_tag_number`, `scales`, `otoliths`, `week`, `section_group_1_10`, `section_group_11_15`, `section_group_16_21`, `section_group_22_38`
...
sapply(all_carcasses[, character_cols[! character_cols %in% c("hallprint", "comments", "crew")]], unique)
```
...
all_carcasses_cleaner <- all_carcasses |> 
  ...
  mutate(spawning_condition = case_when(spawning_condition == "u" ~ "unspawned",
         ...
         adipose_fin_clipped = case_when(adipose_fin_clipped == "y" ~ "TRUE",
                                         ...
                                         adipose_fin_clipped == "k" ~ NA_character_,
         ...
         tag_color = case_when(tag_color == "b" ~ "blue",
                                ...
                                TRUE ~ tag_color)) |>
  select(-c(time, section_group_1_10, section_group_11_15, section_group_16_21, section_group_22_38, morale, crew)) |>  # time is removed because no stop time provided so information is not useful. Not sure section_group variables are useful
  glimpse()
```
...
chop_header_cleaner <- chop_header |> 
  ...
  mutate(crew = str_replace_all(crew, " ", ""),
         ...
         crew = str_replace_all(crew, "&", ","),
         weather = if_else(weather == "cld", "cloudy", weather)) |> 
  select(-time) 
...
* New table/layout structure. There is only a bulk chops table, `ChopJoin`, and a CWT table with individual carcass sampling data.
...
```{r}
all_carcasses_2011 <- all_carcasses_cleaner |> glimpse()
```

```{r, include=FALSE}
f <- function(input, output) write_csv(input, file = output)

gcs_upload(all_carcasses_2011,
           object_function = f,
           type = "csv",
           name = "adult-holding-redd-and-carcass-surveys/feather-river/data/feather_carcass_all_carcasses_2011.csv")
```
