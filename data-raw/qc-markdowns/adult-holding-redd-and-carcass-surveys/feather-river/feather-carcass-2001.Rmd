...
ChopEnv_raw <- read_csv(here::here("data-raw", "qc-markdowns", "adult-holding-redd-and-carcass-surveys",
...
chop_recovery_join <- full_join(ChopEnv_raw %>% select(Date, Time, ChopEnvID), 
                                ChopRecov_raw) %>% 
  clean_names() %>% 
...
```{r}
tag_join <- left_join(TagData_raw, RecovTag_raw) %>% #TODO: what are we joining by? What are these data for? I think tag data is the biological data recorded when the tag was put in, and recovery tag records when that tag has been recovered (?) 
  clean_names() %>% 
...
total_tags_summary <- all_tags %>% 
  ...
  summarise(total_tags = sum(total_tags),
            ...
            unknown_tags = sum(unknown_tags)) |> glimpse()
```
```{r}
total_tags_summary %>% 
...
# TODO: do we need these analyses?
tags_with_dates <- left_join(all_tags, chop_recovery_join) %>% glimpse # TODO: joining on recov_id ?
...
```
```{r}
total_carcass <- full_join(total_chops_summary, total_tags_with_dates_summary) %>% # join on date
...
There are significantly fewer tags than chops on a given day. However a large amount of tags do not have dates to them.
...
```{r}
# TODO: is this analysis necessary?
...
total_cwt_summary <- cwt_count %>% 
  ...
  group_by(month(date)) %>% # TODO: group by month?
...
cwt %>% 
  ...
  geom_boxplot() + 
  theme_minimal() + 
  labs(y = "FL", x = "Sex") # TODO: there is one NA showing up on the plot here
```
...
```
```{r}
unique(chop_recovery_join$chan) # TODO: is this channel? 
```
...
```{r} 
# TODO: deal with NA time
chop_header |> tabyl(time)
...
```{r}
cwt_cleaner <- cwt %>% 
  mutate_if(is.character, str_to_lower) %>%  # TODO: took out renaming "date" column to "datetime" - OK? 
  glimpse
```

## Comments

* TagData_raw and RecovTag_raw both contain Tag 1, Tag 2, TagComp, and Relstat columns. These columns are not ubiquitous amongst tag dataset and there are no look up tables for them. Those columns have been dropped.
...
chop_counts_data_dictionary <- tibble(variables = colnames(chop_recovery_cleaner),
                          description = c("Date of survey",
                                          ...
                                          "Chan", 
                                          "Minute",
                                          "Carcasses that were chopped and sex was male",
                                          "Carcasses that were chopped and sex was female",
                                          "Carcasses that were chopped and grilse",
                                          "Recovery ID"),
...
chop_header_data_dictionary <- tibble(variables = colnames(chop_header_cleaner),
                          description = c("ID",
                                          "Crew member initials that collected",
                                          "Crew member who recorded",
