...
all_carcasses_cleaner <- all_carcasses |> 
  ...
  select(-c(section_group_1_10, section_group_11_15, section_group_16_21,
            section_group_22_38, tag)) |>  # Not sure section_group variables are useful, tag is empty
  glimpse()
```
...
chop_header_cleaner <- chop_header |> 
  ...
  mutate(crew = str_replace_all(crew, " ", ""),
         ...
         weather = case_when(weather %in% c("cld", "frumpy, oh you know") ~ "cloudy",
                             ...
                             weather %in% c("cld,rain", "cld ran", "cld/rain") ~ "cloudy",
                             weather == "clr" ~ "sun",
                             TRUE ~ weather)) |> 
  select(-time) |> 
  glimpse()
...
* `Hallprint` variable no longer available in `cwt` (collected for 2004 - 2014).
