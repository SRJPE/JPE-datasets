...
all_tags <- tag_join %>% 
  ...
  rename("unknown_tags" = X,
         "male_tag" = M,
         "female_tag" = F) %>% 
  select(-c(rel_loc, rel_loc2, rel_stat, comment))
...
total_tags_summary %>% 
  pivot_longer(cols = c(male_tag, female_tag, unknown_tags), names_to = "sex", values_to = "count") %>% 
...
tags_with_dates <- left_join(tag_join %>% 
  ...
  rename("unknown_tags" = X,
         "male_tag" = M,
         "female_tag" = F) %>% 
  select(-c(rel_loc, rel_loc2, rel_stat, comment)), chop_recovery_join)%>% 
...
#   mutate(datetime = ymd_hms(paste(date, hms(start))))%>% 
#   select(-c()) %>% 
#   glimpse
```
