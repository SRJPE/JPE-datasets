...
## Chops and Tags
* Note that within the Chops and Tags data some years have tables titled `chops` while others are titled `chops_and_tags`. For 2017-2021 data, `chops` and `chops_and_tags` are separate tables and here we read in `chops_and_tags` for the most overlap of variables.

### 2000 Chops and Tags
* 2000 and 2001 chops and tags are in different tables so they have to be joined.
...
* 2017-2021 Chop Recovery data contains variables not included in the other chop recovery tables like `sex` and `fork length`. It does not contain `channel_id`, `min`, or `recov_color`.
```{r, message = FALSE}
