...
cleaner_chopTBL %>% 
  mutate(total_chops = unknown_chops + male_chop + female_chop) %>% 
  filter(date == as_date("2010-09-07")) %>% 
  select(chan_id, sect, week_num, date, time, total_chops)

# total number of chops
total_chops  <- cleaner_chopTBL %>% 
  mutate(total_chops = unknown_chops + male_chop + female_chop) %>% 
  group_by(date) %>%
...
# TODO: Look into NA in date
# Potential issue: Reading in excel can introduce NA -> Go back to excel workout and identify if NA for date
# Potential issue: Joins introduces NA
# TODO: Resolve NA and document solution (if able to)

# sum should be 58
sum(total_chops$total_chops == (total_chops$male_chops + total_chops$female_chops + total_chops$unknown_chops))

total_chops %>% 
  filter(total_chops != (male_chops + female_chops + unknown_chops))

cleaner_chopTBL %>% 
  mutate(total_chops = unknown_chops + male_chop + female_chop) %>% 
  filter(total_chops != (male_chop + female_chop + unknown_chops))
