...
chop_channel_join <- full_join(ChopHeader_raw |> 
                                 select(HeaderID = headerID, TagCol, Date),
                               ChopChannel_raw,
                               by = "HeaderID") |> 
  clean_names()
...
chop_recovery_join <- full_join(chop_channel_join |> 
                                  select(date, sect, chan, min, chan_id, header_id,
...
# some dates are NAs - is this from merge?
A = left_join(ChopChannel_raw, ChopHeader_raw |> rename(HeaderID = headerID), 
              by = "HeaderID")$ChanID
B = ChopRecov_raw$ChanID
sum(B %in% A) 

# Where do NA dates come from? Could be through reading in excel (not case here), NAs introduced in join (this is the case; see code above), or could be due to sampling protocol (data periodically collected 2-3 weekdays per week, no specific day)
...
chop_header_cleaner <- chop_header |> 
  ...
  mutate(date = as_date(date),
         ...
         weather = case_when(weather == "cld" ~ "cloudy",
                             weather == "ran" ~ "rain",
                             TRUE ~ weather)) |> 
  select(-c(time, week_num)) |> glimpse()
```
...
cwt_cleaner <- cwt |> 
  ...
  mutate(crew = str_replace_all(crew, " ", ","),
         ...
         sex = case_when(sex == "nd" ~ NA_character_,
                         sex == "uk" ~ "unknown", 
                         sex == "m" ~ "male",
                         sex == "f" ~ "female",
                         TRUE ~ sex), 
