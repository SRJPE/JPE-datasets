...
cwt <- cwt_raw |>  
  left_join(cwt_header_raw,
            by = c("HeaderID" = "headerID")) |> 
  clean_names() |>
  mutate(source_table = "CWT",
         count = 1) |>  
  rename(adipose_fin_clip = clip) |> 
  select(-c(comment, header_id, samp_num)) |> 
  glimpse()
```

### Bulk chop counts

The `chops_and_tags` table has chop counts and tag counts by sex. `disposition` column added to distinguish between chopped and tagged counts.
...
chops <- left_join(ChopChannel_raw, 
  ...
  pivot_longer(c(unknown_chops, male_chop, female_chop), 
               names_to = "sex",
               values_to = "count") |> 
  mutate(disposition = "chopped",
         sex = case_when(sex == "unknown_chops" ~ "unknown",
                         sex == "male_chop" ~ "male",
...
tags <- left_join(ChopChannel_raw, 
  ...
  pivot_longer(c(unknown_tags, female_tag, male_tag), 
               names_to = "sex",
               values_to = "count") |> 
  mutate(disposition = "tagged",
         sex = case_when(sex == "unknown_tags" ~ "unknown",
                         sex == "male_tag" ~ "male",
...
chop_recovery_join <- left_join(ChopRecov_raw |> rename(chan_id = ChanID),
  ...
  select(-c(chan_id, header_id, time)) |> 
  glimpse()
```
...
unique(all_carcasses$hallprint2)
```
...
all_carcasses_cleaner <- all_carcasses |> 
  ...
  select(-c(time, crew, comments)) |>  # time is removed because no stop time provided so information is not useful. chan is all NAs
  glimpse()
```
...
chop_header_cleaner <- chop_header |> 
  ...
  mutate(crew = str_replace_all(crew, " ", ""),
         ...
         weather = case_when(weather == "ran" ~ "rain",
                             weather == "cld" ~ "cloudy",
                             TRUE ~ weather)) |>  
  select(-comments) # stop is all NA and start is not useful without stop

```

## Comments

* Table layout/structure same as 2002-2009.
* `Hallprint` variable in `cwt` (began collecting in 2004).
* no more `run` variable (was collected in 2008 and 2009)
* `ChopChannel` table this year does not contain lifestage or fin clip information.
...
```{r}
all_carcasses_2010 <- all_carcasses_cleaner |> glimpse()
```

```{r, include=FALSE}
f <- function(input, output) write_csv(input, file = output)

gcs_upload(all_carcasses_2010,
           object_function = f,
           type = "csv",
           name = "adult-holding-redd-and-carcass-surveys/feather-river/data/feather_carcass_all_carcasses_2010.csv")
```
