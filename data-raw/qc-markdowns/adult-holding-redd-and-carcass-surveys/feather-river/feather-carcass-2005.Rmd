...
# MDBPATH <- "C:/Users/InigoPeng/Projects/jpe/JPE-datasets/data-raw/qc-markdowns/adult-holding-redd-and-carcass-surveys/feather-river/Carcass 2006.mdb"
...
# write_csv(ChopChannel_raw, "ChopChannel_2006.csv")
# 
...
ChopRecov_raw <- read_excel("ChopRecov_2005.xlsx") %>% glimpse

ChopHeader_raw <- read_excel("ChopHeader_2005.xlsx") %>% 
  rename("ID" = headerID) %>%
  glimpse()

cwt_raw <- read_excel("CWTTag_2005.xlsx") %>% glimpse()

cwt_header_raw <- read_csv("CWTHeader_2005.xlsx")
```
...
#the 2005 access database contains data from 2004 as well - need to filter for only 2005
...
chop_channel_cleaner <- chop_channel_join %>%
  ...
  select(-c(date, time, male, fem, gril, tag_count))
