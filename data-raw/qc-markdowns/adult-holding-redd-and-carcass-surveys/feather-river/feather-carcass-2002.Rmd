...
### cwt_raw
The CWT table contains TagIDs for carcasses with sample information and a variable `CarcStat` that indicates whether the carcass was chopped (C) or tagged (T). 
```{r}
...
### ChopEnv_Raw
This table contains total chops by sex.  
```{r}
...
### ChopRecov_raw
This table contains recovered carcasses by channel ID. 
```{r}
...
The `chop_channel` table contains chop counts by sex and a tag count column.
```{r}
...
The `cwt` table contains sampling information on individual carcasses, including whether the carcass was tagged or chopped. Unclear whether a single `tag_id` will appear multiple times in the dataset based; there are multiples, but they seem to be mostly duplicates.
```{r}
...
cwt_cleaner <- cwt |> 
  ...
  mutate(crew = str_replace_all(crew, " ", ""),
         ...
         spawn = case_when(spawn %in% c("no data", "nd") ~ NA_character_,
                           spawn == "u" ~ "unspawned",
                           spawn == "s" ~ "spawned",
                           spawn == "uk" ~ "unknown",
                           TRUE ~ spawn),
