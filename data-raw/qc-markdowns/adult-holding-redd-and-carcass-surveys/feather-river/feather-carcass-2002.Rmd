...
cwt <- cwt_raw |>  
  left_join(cwt_header_raw |>
              select(HeaderID = headerID, date = Date),
            by = "HeaderID") |> 
  clean_names() |>
...
### All Carcasses Variables: `header_id`, `chan_id`, `sect`, `min`, `count`, `recov_id`, `count`, `recov_id`, `tag_id`, `fl`, `head_tag`
...
total_grilse_chops_summary |>
  ...
  scale_fill_manual(values = wes_palette("Moonrise2"))
```
This plot shows the sex proportion of grilse chops for each day over the the period of 2002-09-03 to 2002-12-19. 2002 data did not separate out grilse chops by sex so all are `NA` here. There are more data gaps for the grilse data compared to the normal chops data.
...
* Tables change from 2000-2001 (which had `TagData`, `ChopRecov`, `RecovTag`, and `CWT`) to now having `ChopRecov`, `ChopChannel`, and `CWT`. 
...
```{r}
all_carcasses_2002 <- all_carcasses_cleaner |> glimpse()
```
```{r, include=FALSE}
f <- function(input, output) write_csv(input, file = output)

gcs_upload(all_carcasses_2002,
           object_function = f,
           type = "csv",
           name = "adult-holding-redd-and-carcass-surveys/feather-river/data/feather_carcass_all_carcasses_2002.csv")
```
