...
### read in updated data (1997-2023)
```{r}
...
# join back in starting elevation data (where we have it)
starting_elevations <- raw_data |> 
  select(starting_elevation = `Starting Elevevation`,
         location = Section) |> 
  mutate(starting_elevation = as.numeric(str_remove_all(starting_elevation, "'"))) |> 
  glimpse()

cleaner_data <- cleaner_data_raw |> 
  left_join(starting_elevations, by = "location") |> 
  glimpse()
```
