...
We looked at USGS and only mean data is available starting 2016, CDEC has finer resolution hourly data, that is why we decided to use CDEC. 
We are using USGS data to help filter out outliers.

### Using USGS mean min and max to filter out outliers from CDEC
```{r}
#USGS
BCK_USGS <- readNWISdv(11390000, "00010") # parameter code 00010: water temperature, mean 
...
#applying USGS min and max to remove outliers from CDEC data
...
combined_data <- bind_rows(
  mutate(BCK_USGS_max, HourlyTemperatures = "Max Temp"),
  mutate(BCK_USGS_min, HourlyTemperatures = "Min Temp")
)
