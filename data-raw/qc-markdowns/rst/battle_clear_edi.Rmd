...
res <- read_data_entity_names(packageId = version)
# catch
raw <- read_data_entity(packageId = version, entityId = res$entityId[1])
catch <- readr::read_csv(file = raw)
#trap
raw <- read_data_entity(packageId = version, entityId = res$entityId[2])
trap <- readr::read_csv(file = raw)
#recapture
raw <- read_data_entity(packageId = version, entityId = res$entityId[3])
recapture <- readr::read_csv(file = raw)
#release
raw <- read_data_entity(packageId = version, entityId = res$entityId[4])
...
trap_format <- trap |> 
  ...
  # this does not completely work so second step would be to correct with the assumption
  # that trap stop date is one day after the start date
         ...
         trap_start_date = case_when(diff > 7 | diff < -7 ~ as_date(trap_stop_date - 1), 
                                     T ~ trap_start_date)) %>% 
  select(-debris_tubs, -avg_time_per_rev, -diff) %>%
