...
This document will only work on Mac OS. Currently there is a local file path which
...
environmental <- mdb.get(tisdale_access_db, tables = "EnvDataRaw")
```
# Lookup tables
```{r}
# pull lookup tables
...
unit_lu <- mdb.get(tisdale_access_db, "luUnit") %>% 
   select(-activeID) %>% glimpse()
