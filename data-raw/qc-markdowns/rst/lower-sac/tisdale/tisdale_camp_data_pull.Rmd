...
trap_visit_format <- trap_visit %>% 
  ...
  mutate(stream = case_when(projectDescriptionID == 10 ~ "sacramento river",
                            projectDescriptionID == 12 ~ "feather river"),
         site = case_when(projectDescriptionID == 10 ~ "tisdale",
                            projectDescriptionID == 12 ~ "lower feather river"))
...
selected_recaptured <- catch_raw %>% 
  select(projectDescriptionID, catchRawID, taxonID, atCaptureRunID, atCaptureRunMethodID, finalRunID, finalRunMethodID, fishOriginID, lifeStageID, forkLength, weight, n, trapVisitID, releaseID, mortID) %>% 
...
selected_release_fish <- release_fish %>% 
  ...
  full_join(selected_release %>% 
  select(projectDescriptionID, stream, site, releaseID))
  
f <- function(input, output) write_csv(input, file = output)
gcs_upload(selected_release_fish,
           object_function = f,
           type = "csv",
           name = "rst/lower-sac-river/data-raw/tisdale/tisdale_release_fish_camp.csv",
           predefinedAcl = "bucketLevel")  
```
...
discharge <- environmental %>% 
  select(projectDescriptionID, trapVisitID, envDataRawID, discharge, dischargeUnitID, dischargeSampleGearID) %>% 
...
depth <- environmental %>% 
  select(projectDescriptionID, trapVisitID, envDataRawID, waterDepth, waterDepthUnitID, waterDepthSampleGearID) %>% 
...
velocity <- environmental %>% 
  select(projectDescriptionID, trapVisitID, envDataRawID, waterVel, waterVelUnitID, waterVelSampleGearID) %>% 
...
temperature <- environmental %>% 
  select(projectDescriptionID, trapVisitID, envDataRawID, waterTemp, waterTempUnitID, waterTempSampleGearID) %>% 
...
turbidity <- environmental %>% 
  select(projectDescriptionID, trapVisitID, envDataRawID,  turbidity, turbidityUnitID, turbiditySampleGearID) %>% 
...
oxygen <- environmental %>% 
  select(projectDescriptionID, trapVisitID, envDataRawID,  dissolvedOxygen, dissolvedOxygenUnitID, dissolvedOxygenSampleGearID) %>% 
...
conductivity <- environmental %>% 
  select(projectDescriptionID, trapVisitID, envDataRawID,  conductivity, conductivityUnitID, conductivitySampleGearID) %>% 
...
barometer <- environmental %>% 
  select(projectDescriptionID, trapVisitID, envDataRawID,  barometer, barometerUnitID, barometerSampleGearID) %>% 
...
trap_visit_subsite <- trap_visit %>%
  select(projectDescriptionID, trapVisitID, visitTime, visitTime2, trapPositionID) %>%
  left_join(site_subsite, by = c("trapPositionID" = "subSiteID")) %>% 
  select(-trapPositionID, -siteID) %>% 
...
environmental_format <- bind_rows(discharge,
  ...
  # remove cases where projectDescriptionID is NA, these usually have a value of NA
  filter(!is.na(projectDescriptionID)) %>% 
  mutate(unit = case_when(is.na(unit) & !is.na(value) ~ "not recorded", 
         ...
         sample_gear = case_when(is.na(sample_gear) & !is.na(value) ~ "not recorded", 
                          ...
                          T ~ sample_gear),
         stream = case_when(projectDescriptionID == 10 ~ "sacramento river",
                            projectDescriptionID == 12 ~ "feather river"),
         site = case_when(projectDescriptionID == 10 ~ "tisdale",
                            projectDescriptionID == 12 ~ "lower feather river")) %>% glimpse()
