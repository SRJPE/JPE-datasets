...
model_format_upstream <- standard_adult_upsream %>% 
  ...
  group_by(location, year, passage_direction, run, sex, origin) %>% 
  summarize(count = sum(count, na.rm = TRUE)) %>%
  select(location, year, count, passage_direction, run, sex, origin) %>% glimpse
```
...
model_format_upstream %>% 
  ggplot() + 
  geom_col(aes(x = year, y = count, fill = location), position = "dodge") + 
...
model_format_upstream %>% 
  ggplot() + 
  geom_col(aes(x = year, y = count, fill = location)) + 
  labs(title = "total yearly upstream passage count") +
  scale_fill_manual(values = color_pal) + 
  theme_minimal() + 
  theme(legend.position = "bottom", 
...
model_format_upstream %>% 
  ggplot() + 
  geom_col(aes(x = year, y = count, fill = run)) + 
  labs(title = "total yearly upstream passage count by run") +
  scale_fill_manual(values = color_pal) + 
  theme_minimal() + 
  theme(legend.position = "bottom", 
        text = element_text(size = 18),
...
model_format_upstream %>% 
  ...
  labs(title = "range of count per year by tributatry ") +
